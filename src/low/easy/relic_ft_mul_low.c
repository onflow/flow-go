...
void ft_mul1_low(dig_t *c, dig_t *a, dig_t digit) {
	...
	c[FB_DIGS] = ft_lshb_low(c, a, util_bits_dig(digit) - 1);
...
void ft_muln_low(dig_t *c, dig_t *a, dig_t *b) {
	...
	for (i = FB_DIGIT - 4; i > 0; i -= 4) {
		...
		for (j = 0; j < FB_DIGS; j++, tmpa++, tmpc++) {
			u = (*tmpa >> i) & 0x0F;
			ft_addn_low(tmpc, tmpc, table[u]);
			*(tmpc + FB_DIGS) ^= table[u][FB_DIGS];
		}
		carry = ft_lshb_low(c, c, 4);
		ft_lshb_low(c + FB_DIGS, c + FB_DIGS, 4);
		c[FB_DIGS] ^= carry;
	}
	for (j = 0; j < FB_DIGS; j++, a++, c++) {
		u = *a & 0x0F;
		ft_addn_low(c, c, table[u]);
