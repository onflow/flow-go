...
static void wtnaf_mod(bn_t r0, bn_t r1, bn_t k, bn_t vm, bn_t s0, bn_t s1,
		int u, int m) {
	bn_t t0 = NULL, t1 = NULL, t2 = NULL, t3 = NULL;
	int a, n, n0, n1, h0, h1;
	dig_t d0, d1;
	...
	/* t3 = (g' + j' + 1/2 * 2^(m + 5)/2. */
	bn_add(t3, t3, t0);
	/* t_0 = 2^C * l_i = floor((g' + j')/2^(m + 5)/2 + 1/2). */
	bn_rsh(t0, t3, (m + 5) / 2);
	...
	/* t3 = (g' + j' + 1/2 * 2^(m + 5)/2. */
	bn_add(t3, t3, t1);
	/* t_1 = 2^C * l_i = floor((g' + j')/2^(m + 5)/2 + 1/2). */
	bn_rsh(t1, t3, (m + 5) / 2);

	/*
	 * Both l_0 and l_1 should be rational, but we keep them as integers
	 * multiplied by 2^C. Now we can round off l_0 and l_1.
	 */
	...
	/* Now we repeat the same for q_1. */
	bn_get_dig(&d1, t1);
	n1 = d1 & MOD_CMASK;
	if (n1 >= MOD_2TC / 2) {
		n1 -= MOD_2TC;
