...
function activate(ctx) {
  ...
  function registerCommand(command, callback) {
    ctx.subscriptions.push(
      vscode_1.commands.registerCommand(command, callback)
    );
  }
  let client = startServer(ctx);
  registerCommand("bamboo.restartServer", () =>
...
function startServer(ctx) {
  ...
  if (!serverBinaryPath) {
    vscode_1.window.showWarningMessage(
      "Missing path to Bamboo language server"
    );
    return;
  }
  ...
  client
    ...
    .then(() => {
      return vscode_1.window.showInformationMessage(
        "Bamboo language server started"
      );
    })
    .catch(error => {
