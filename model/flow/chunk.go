...
type ChunkBody struct {
	...
	EventCollection model.Fingerprint // Events generated by executing results
...
// Fingerprint provides a cryptographic commitment for a chunk content
func (ch *Chunk) Fingerprint() model.Fingerprint {
	return model.Fingerprint(hash.DefaultHasher.ComputeHash(encoding.DefaultEncoder.MustEncode(ch)))
}

// ID returns a unique id for this entity
func (ch *Chunk) ID() model.Identifier {
	var id model.Identifier
	copy(id[:], ch.Fingerprint())
	return id
}
...
func (cl *ChunkList) Fingerprint() model.Fingerprint {
...
func (cl *ChunkList) ByFingerprint(c model.Fingerprint) Chunk {
	for _, item := range cl.chunks {
