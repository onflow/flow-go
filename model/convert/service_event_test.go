...
func TestEventConversion(t *testing.T) {
	...
	// 4 cluster with 1 node each
	clusters := [][]string{{"e9ffa1712474abc92c48e209051f335b1d9b29226f8a9471dabcaa53f62d3746"},
		{"bef02e1a8ca72dd27e78650bcb793e6cbc54549f785232c0bb45099f25ee50db"},
		{"76a1a261f463d1530f1db90d412c0e68a1516aa740cb39b977c4bbe8aafc9afe"},
		{"e81b23c32d05b038f67eb90a0538b45242e43bcc56530554b7b3f3d354b08f2e"}}
	...
	t.Run("epoch setup", func(t *testing.T) {
		...
		firstView := uint(70)
		finalView := uint(139)
		randomSource, err := hex.DecodeString("15800823147682539441")
		...
		testassert.Equal(t, actual.Counter, counter)
		testassert.Equal(t, actual.FirstView, firstView)
		testassert.Equal(t, actual.FinalView, finalView)
		testassert.Equal(t, actual.RandomSource, randomSource)

		// make sure assignment lengths are equal
		testassert.Len(t, actual.Assignments, len(clusters))
		...
		for index, assignment := range actual.Assignments {
			...
			testassert.Len(t, assignment, len(cluster))

			// check if node ids are read in correctly
			for index, nodeID := range assignment {
				requiredNodeID, err := flow.HexStringToIdentifier(cluster[index])
				require.NoError(t, err)
				testassert.Equal(t, requiredNodeID, nodeID)
			}
		}
