...
func generateQCs(nodesPerCluster, nClusters int) {

	msg := unittest.RandomBytes(128)
	fmt.Printf("%x\n", msg)

	for cluster := 0; cluster < nClusters; cluster++ {
		var sigs []crypto.Signature
		for node := 0; node < nodesPerCluster; node++ {
			key := unittest.KeyFixture(crypto.BLSBLS12381)
			id := unittest.IdentityFixture(unittest.WithStakingPubKey(key.PublicKey()))
			local, err := local.New(id, key)
			if err != nil {
				panic(err)
			}

			agg := signature.NewAggregationProvider(encoding.CollectorVoteTag, local)

			sig, err := agg.Sign(msg)
			if err != nil {
				panic(err)
			}
		...
		agg := signature.NewAggregationProvider("", nil)
		aggregated, err := agg.Aggregate(sigs)
		if err != nil {
			panic(err)
		}
		...
		fmt.Println("Agg sig: ", aggregated.String())
		fmt.Println()
	}
}
