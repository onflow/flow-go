...
func Not(filter libp2p.PeerFilter) libp2p.PeerFilter {
	return func(p *libp2p.Peer) bool {
		return !filter(p)
	}
}

// HasSeen filters for peers that have seen the event with the given ID.
func HasSeen(eventID []byte) libp2p.PeerFilter {
...
// IDs.
func HasSeenAllOf(eventIDs ...[]byte) libp2p.PeerFilter {
...
// given IDs.
func HasSeenOneOf(eventIDs ...[]byte) libp2p.PeerFilter {
	keys := make([]string, 0, len(eventIDs))
	for _, eventID := range eventIDs {
		key := hex.EncodeToString(eventID)
		keys = append(keys, key)
	}
	return func(i *libp2p.Peer) bool {
		for _, key := range keys {
			_, ok := i.Seen[key]
			if ok {
				return true
