...
func (stt *SparseTopologyTestSuite) TestSparselyConnectedNetwork() {
	...
	for _, n := range stt.nets {
		eng := NewMeshEngine(stt.Suite.T(), n, count-1, 1)
...
// TestDisjointedNetwork creates a network configuration with of 9 nodes with 3 subsets. The subsets are not connected
// with each other.
// 0,1,2 <-> 3,4,5 <-> 6,7,8
...
func CreateSparseTopology(count int, subsets int) []middleware.Topology {
	...
	for i := 0; i < count; i++ {
		...
		if s == subsets-1 { // if this is the last subset
			maxIndex = count // then set max index to count + 1 since nodes so may not evenly divide by # of subsets
		} else {
...
// checkMessageReception waits till receivedBy number of engines have received a message or times out after 10 second
