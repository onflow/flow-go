...
func (r *RcvCacheTestSuite) TestSingleElementAdd() {
	...
	channelID := uint32(0)
	assert.False(r.Suite.T(), r.c.ContainsOrAdd(eventID, channelID))

	assert.True(r.Suite.T(), r.c.ContainsOrAdd(eventID, channelID))
}
...
func (r *RcvCacheTestSuite) TestMultipleElementAdd() {
	...
	for i := range events {
		assert.False(r.Suite.T(), r.c.ContainsOrAdd(events[i], uint32(i)))
	}

	// checks for the existence of the added events
	for i := range events {
		assert.True(r.Suite.T(), r.c.ContainsOrAdd(events[i], uint32(i)))
	}
}
