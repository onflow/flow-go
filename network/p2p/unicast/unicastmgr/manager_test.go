...
func TestUnicastManager_Connection_SuccessfulConnection_And_Stream(t *testing.T) {
	...
	require.Equal(t, uint64(unicastmodel.MaxStreamCreationAttempt), dialCfg.StreamBackBudget) // stream backoff budget must remain intact.
...
// It tests that when there is a connection, but no stream, it tries to create a stream some number of times (unicastmodel.MaxStreamCreationAttempt), before
// giving up.
...
func TestUnicastManager_Connection_SuccessfulConnection_StreamBackoff(t *testing.T) {
	...
	isConnectedCalled := 0
	connStatus.On("IsConnected", peerID).Return(
	...
	dialCfg, err := dialConfigCache.GetOrInit(peerID)
	require.NoError(t, err)
	require.Equal(t, uint64(unicastmodel.MaxConnectAttempt), dialCfg.DialBackoffBudget)         // dial backoff budget must be intact, since the connection is successful.
	require.Equal(t, uint64(unicastmodel.MaxStreamCreationAttempt-1), dialCfg.StreamBackBudget) // stream backoff budget must be decremented by 1 since all budget is used up.
	// last successful dial must be set AFTER the successful dial.
	require.True(t, dialCfg.LastSuccessfulDial.After(dialTime))
...
func TestUnicastManager_Stream_BackoffBudgetResetToDefault(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, uint64(unicastmodel.MaxConnectAttempt), dialCfg.DialBackoffBudget)       // dial backoff budget must be intact.
	require.Equal(t, uint64(unicastmodel.MaxStreamCreationAttempt), dialCfg.StreamBackBudget) // stream backoff budget must reset to default.
...
func TestUnicastManager_Stream_BackoffConnectionBudgetResetToDefault(t *testing.T) {
	...
	dialCfg, err := dialConfigCache.GetOrInit(peerID)
	require.NoError(t, err)
	require.Equal(t, uint64(unicastmodel.MaxConnectAttempt), dialCfg.DialBackoffBudget)       // dial backoff budget must be reset to default.
	require.Equal(t, uint64(unicastmodel.MaxStreamCreationAttempt), dialCfg.StreamBackBudget) // stream backoff budget must be intact.
...
func TestUnicastManager_Connection_NoBackoff_When_Budget_Is_Zero(t *testing.T) {
	...
	adjustedCfg, err := dialConfigCache.Adjust(
		peerID, func(dialConfig unicastmodel.DialConfig) (unicastmodel.DialConfig, error) {
			dialConfig.DialBackoffBudget = 0                                  // set the dial backoff budget to 0, meaning that the dial backoff budget is exhausted.
	...
	dialCfg, err := dialConfigCache.GetOrInit(peerID)
	require.NoError(t, err)
	require.Equal(t, uint64(0), dialCfg.DialBackoffBudget)                                    // dial backoff budget must remain at 0.
	require.Equal(t, uint64(unicastmodel.MaxStreamCreationAttempt), dialCfg.StreamBackBudget) // stream backoff budget must be intact.
...
func TestUnicastManager_Stream_NoBackoff_When_Budget_Is_Zero(t *testing.T) {
	...
	adjustedCfg, err := dialConfigCache.Adjust(
		peerID, func(dialConfig unicastmodel.DialConfig) (unicastmodel.DialConfig, error) {
			...
			dialConfig.StreamBackBudget = 0                    // set the stream backoff budget to 0, meaning that the stream backoff budget is exhausted.
			return dialConfig, nil
		},
	)
	require.NoError(t, err)
	...
	dialCfg, err := dialConfigCache.GetOrInit(peerID)
	require.NoError(t, err)
	require.Equal(t, uint64(unicastmodel.MaxConnectAttempt), dialCfg.DialBackoffBudget) // dial backoff budget must remain intact.
	require.Equal(t, uint64(0), dialCfg.StreamBackBudget)                               // stream backoff budget must remain zero.
