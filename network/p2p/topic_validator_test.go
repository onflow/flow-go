...
func TestAuthorizedSenderValidator_UnAuthorized(t *testing.T) {
	...
	require.Equalf(t, 1, hookCalls, "expected 1 warning to be logged")
}
...
// TestAuthorizedSenderValidator_InvalidWeight tests that the authorized sender validator rejects messages from nodes with invalid weight
func TestAuthorizedSenderValidator_InvalidWeight(t *testing.T) {
	sporkId := unittest.IdentifierFixture()
	identity1, privateKey1 := unittest.IdentityWithNetworkingKeyFixture(unittest.WithRole(flow.RoleConsensus))
	sn1 := createNode(t, identity1.NodeID, privateKey1, sporkId)
	...
	// expecting 1 warn calls for each rejected message from node with invalid weight
	require.Equalf(t, 1, hookCalls, "expected 1 warning to be logged")
}
