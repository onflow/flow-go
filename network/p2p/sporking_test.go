...
func TestOneToKCrosstalkPrevention(t *testing.T) {
	...
	_, err = node1.Subscribe(topicBeforeSpork, unittest.NetworkCodec(), unittest.IsStakedTopicValidatorFunc())
	require.NoError(t, err)
	sub2, err := node2.Subscribe(topicBeforeSpork, unittest.NetworkCodec(), unittest.IsStakedTopicValidatorFunc())
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = node1.Subscribe(topicAfterSpork, unittest.NetworkCodec(), unittest.IsStakedTopicValidatorFunc())
	require.NoError(t, err)
