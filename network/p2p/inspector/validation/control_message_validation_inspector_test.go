...
import (
	...
	p2ptest "github.com/onflow/flow-go/network/p2p/inspector/internal"
...
func TestControlMessageValidationInspector_truncateRPC(t *testing.T) {
	t.Run("truncateGraftMessages should truncate graft messages as expected", func(t *testing.T) {
		inspector, _, _, _, _, _ := inspectorFixture(t)
	...
	t.Run("truncateIWantMessages should truncate iWant messages as expected", func(t *testing.T) {
		inspector, _, rpcTracker, _, _, _ := inspectorFixture(t)
...
func TestControlMessageValidationInspector_processInspectRPCReq(t *testing.T) {
	t.Run("processInspectRPCReq should not disseminate any invalid notification errors for valid RPC's", func(t *testing.T) {
		inspector, distributor, rpcTracker, _, topicProvider, sporkID := inspectorFixture(t)
		...
		req, err := NewInspectRPCRequest(expectedPeerID,
			unittest.P2PRPCFixture(unittest.WithGrafts(grafts...),
	...
	t.Run("processInspectRPCReq should disseminate invalid control message notification for control messages with duplicate topics", func(t *testing.T) {
		inspector, distributor, _, _, _, sporkID := inspectorFixture(t)
		defer distributor.AssertNotCalled(t, "Distribute")
	...
	t.Run("inspectIWantMessages should disseminate invalid control message notification for iWant messages when duplicate message ids exceeds the allowed threshold",
		func(t *testing.T) {
	...
	t.Run("inspectIWantMessages should disseminate invalid control message notification for iWant messages when cache misses exceeds allowed threshold",
		func(t *testing.T) {
			inspector, distributor, rpcTracker, _, _, _ := inspectorFixture(t)
	...
	t.Run("inspectRpcPublishMessages should disseminate invalid control message notification when invalid pubsub messages count greater than configured RpcMessageErrorThreshold",
		func(t *testing.T) {
			inspector, distributor, _, _, topicProvider, sporkID := inspectorFixture(t)
	...
	t.Run("inspectRpcPublishMessages should disseminate invalid control message notification when publish messages contain no topic", func(t *testing.T) {
		inspector, distributor, _, _, _, _ := inspectorFixture(t)
		// 5 invalid pubsub messages will force notification dissemination
		inspector.config.RpcMessageErrorThreshold = 4
...
func TestNewControlMsgValidationInspector_validateClusterPrefixedTopic(t *testing.T) {
	t.Run("validateClusterPrefixedTopic should not return an error for valid cluster prefixed topics", func(t *testing.T) {
		inspector, distributor, _, idProvider, _, sporkID := inspectorFixture(t)
		defer distributor.AssertNotCalled(t, "Distribute")
		...
		require.NoError(t,
			inspector.validateClusterPrefixedTopic(from,
				clusterPrefixedTopic,
				flow.ChainIDList{clusterID, flow.ChainID(unittest.IdentifierFixture().String()), flow.ChainID(unittest.IdentifierFixture().String())}))
	})
...
func TestControlMessageValidationInspector_ActiveClustersChanged(t *testing.T) {
	...
	inspector, err := NewControlMsgValidationInspector(signalerCtx,
		unittest.Logger(),
		sporkID,
		...
		func() p2p.TopicProvider {
			return p2ptest.NewMockUpdatableTopicProvider()
		})
	require.NoError(t, err)
...
// Returns:
// - *ControlMsgValidationInspector fixture: used to validate control messages
// - *mockp2p.GossipSubInspectorNotifDistributor fixture: used to disseminate invalid control message notifications
// - *mockp2p.RpcControlTracking fixture: used to track rpc control messages
// - *mockmodule.IdentityProvider fixture: used to provide identity for peer ID
// - *p2ptest.MockUpdatableTopicProvider fixture: used to provide list of topics
// - flow.Identifier fixture: used as spork ID
func inspectorFixture(t *testing.T) (*ControlMsgValidationInspector,
	*mockp2p.GossipSubInspectorNotifDistributor,
	*mockp2p.RpcControlTracking,
	*mockmodule.IdentityProvider,
	*p2ptest.MockUpdatableTopicProvider,
	flow.Identifier) {
	sporkID := unittest.IdentifierFixture()
	flowConfig, err := config.DefaultConfig()
	require.NoError(t, err, "failed to get default flow config")
	...
	signalerCtx := irrecoverable.NewMockSignalerContext(t, context.Background())
	topicProvider := p2ptest.NewMockUpdatableTopicProvider()
	inspector, err := NewControlMsgValidationInspector(signalerCtx,
		unittest.Logger(),
		sporkID,
		&flowConfig.NetworkConfig.GossipSubRPCValidationInspectorConfigs,
	...
	return inspector, distributor, rpcTracker, idProvider, topicProvider, sporkID
}
