...
// It returns (true, nil) if the queue was initialized with the default index.
// It returns (false, nil) if the queue was already initialized.
func (q *ChunksQueue) Init(defaultIndex uint64) (bool, error) {
	latestIndex, err := q.LatestIndex()
	if errors.Is(err, storage.ErrNotFound) {
		...
		q.latestJobIndex = atomic.NewUint64(defaultIndex)
		return true, nil
	}
	if err != nil {
	...
	q.latestJobIndex = atomic.NewUint64(latestIndex)

	return false, nil
}

// StoreChunkLocator stores a new chunk locator that assigned to me to the job queue.
// Note: StoreChunkLocator is not concurrent-safe, making sure only 1 worker is storing
// chunk locator, and multiple workers can read the chunk locator concurrently.
...
// it might return storage.ErrNotFound if it's not initialized
