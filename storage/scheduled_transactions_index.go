...
// ScheduledTransactionIterator is an iterator over scheduled transactions ordered by
// descending ID (highest first).
type ScheduledTransactionIterator = IndexIterator[accessmodel.ScheduledTransaction, accessmodel.ScheduledTransactionCursor]
...
type ScheduledTransactionsIndexReader interface {
	...
	// ByAddress returns an iterator over scheduled transactions for the given account,
	// ordered by descending ID (highest first).
	// Returns an exhausted iterator and no error if the account has no transactions.
	//
	// `cursor` is a pointer to an [accessmodel.ScheduledTransactionCursor]:
	...
	) (ScheduledTransactionIterator, error)

	// All returns an iterator over all scheduled transactions, ordered by descending ID
	// (highest first). Returns an exhausted iterator and no error if no transactions exist.
	//
	// `cursor` is a pointer to an [accessmodel.ScheduledTransactionCursor]:
	//   - nil means start from the highest indexed ID (first page)
	//   - non-nil means start at the cursor position (inclusive)
	...
	// Expected error returns during normal operation:
	//   - [ErrNotBootstrapped]: if the index has not been initialized
	All(cursor *accessmodel.ScheduledTransactionCursor) (ScheduledTransactionIterator, error)
}
