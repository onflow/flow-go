...
func TestCollections_IndexDuplicateTx(t *testing.T) {
	dbtest.RunWithDB(t, func(t *testing.T, db storage.DB) {
		...
		require.NoError(t, err)
		assert.Equal(t, col2Light.Transactions, gotLightByCol2ID.Transactions)
		assert.Equal(t, col2Light.ID(), gotLightByCol2ID.ID())
		...
		require.NoError(t, err)
		assert.Equal(t, col1Light.Transactions, gotLightByDupTxID.Transactions)
		assert.Equal(t, col1Light.ID(), gotLightByDupTxID.ID())
	})
}
