...
func TestEvents_HappyPath(t *testing.T) {
	...
	event1 := unittest.EventFixture(flow.EventAccountCreated, 0, 0, transaction1.ID())
	event2 := unittest.EventFixture(flow.EventAccountUpdated, 0, 1, transaction1.ID())
	event3 := unittest.EventFixture(flow.EventAccountCreated, 1, 2, transaction2.ID())
