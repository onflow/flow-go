...
// TestNFTTransfers_PaginationCoversAllEntries verifies that paginating through all transfers
...
func TestNFTTransfers_PaginationCoversAllEntries(t *testing.T) {
	t.Parallel()
	...
	initialTransfers := []access.NonFungibleTokenTransfer{
		{TransactionID: unittest.IdentifierFixture(), BlockHeight: firstHeight, TransactionIndex: 0, EventIndices: []uint32{0}, SourceAddress: account, RecipientAddress: other, ID: 1},
		{TransactionID: unittest.IdentifierFixture(), BlockHeight: firstHeight, TransactionIndex: 1, EventIndices: []uint32{0}, SourceAddress: account, RecipientAddress: other, ID: 2},
		{TransactionID: unittest.IdentifierFixture(), BlockHeight: firstHeight, TransactionIndex: 2, EventIndices: []uint32{0}, SourceAddress: account, RecipientAddress: other, ID: 3},
	}

	RunWithBootstrappedNFTTransferIndex(t, firstHeight, initialTransfers, func(_ storage.DB, lm storage.LockManager, idx *NonFungibleTokenTransfers) {
		// 3 more transfers at height 6 (one above firstHeight)
		err := storeNFTTransfers(t, lm, idx, 6, []access.NonFungibleTokenTransfer{
			{TransactionID: unittest.IdentifierFixture(), BlockHeight: 6, TransactionIndex: 0, EventIndices: []uint32{0}, SourceAddress: account, RecipientAddress: other, ID: 4},
			{TransactionID: unittest.IdentifierFixture(), BlockHeight: 6, TransactionIndex: 1, EventIndices: []uint32{0}, SourceAddress: account, RecipientAddress: other, ID: 5},
			{TransactionID: unittest.IdentifierFixture(), BlockHeight: 6, TransactionIndex: 2, EventIndices: []uint32{0}, SourceAddress: account, RecipientAddress: other, ID: 6},
		})
		require.NoError(t, err)
		...
		var allCollected []access.NonFungibleTokenTransfer
		var cursor *access.TransferCursor
		for {
			nftIter, err := idx.ByAddress(account, cursor)
			require.NoError(t, err)

			page, nextCursor, err := iterator.CollectResults(nftIter, pageSize, nil)
			require.NoError(t, err)
