...
func TestProtocolStateStorage(t *testing.T) {
	unittest.RunWithBadgerDB(t, func(db *badger.DB) {
		...
		err := transaction.Update(db, func(tx *transaction.Tx) error {
			...
			err = store.StoreTx(protocolStateID, expected.EpochMinStateEntry)(tx)
			require.NoError(t, err)
...
func TestProtocolStateStoreInvalidProtocolState(t *testing.T) {
	unittest.RunWithBadgerDB(t, func(db *badger.DB) {
		...
		commits := NewEpochCommits(metrics, db)
		store := NewEpochProtocolStateEntries(metrics, setups, commits, db, DefaultEpochProtocolStateCacheSize, DefaultProtocolStateIndexCacheSize)
		invalid := unittest.EpochStateFixture().EpochMinStateEntry
		...
		invalid = unittest.EpochStateFixture(unittest.WithNextEpochProtocolState()).EpochMinStateEntry
		// swap first and second elements to break canonical order
...
func TestProtocolStateMergeParticipants(t *testing.T) {
	unittest.RunWithBadgerDB(t, func(db *badger.DB) {
		...
		err := transaction.Update(db, func(tx *transaction.Tx) error {
			...
			return store.StoreTx(protocolStateID, stateEntry.EpochMinStateEntry)(tx)
		})
		require.NoError(t, err)
...
func assertRichProtocolStateValidity(t *testing.T, state *flow.RichEpochProtocolStateEntry) {
	...
	assert.Equal(t, state.CurrentEpochSetup.ID(), state.EpochMinStateEntry.CurrentEpoch.SetupID, "epoch setup should be for correct event ID")
	assert.Equal(t, state.CurrentEpochCommit.ID(), state.EpochMinStateEntry.CurrentEpoch.CommitID, "epoch commit should be for correct event ID")
	...
	if state.PreviousEpoch != nil {
		...
		assert.Equal(t, state.PreviousEpochSetup.ID(), state.EpochMinStateEntry.PreviousEpoch.SetupID, "epoch setup should be for correct event ID")
		assert.Equal(t, state.PreviousEpochCommit.ID(), state.EpochMinStateEntry.PreviousEpoch.CommitID, "epoch commit should be for correct event ID")
	...
	//  - NextEpochSetup and NextEpochCommit IDs match respective commitments in the `EpochMinStateEntry`.
