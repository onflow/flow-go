...
func (mt *MTrie) NewNodeIterator() *NodeIterator {
	return newNodeIterator(mt.root, mt.maxHeight-1)
}

// GetNodes returns all slice containing all nodes of the MTrie. The nodes are
// listed following a DESCENDANTS-FIRST-RELATIONSHIP:
//   * Consider the sequence of nodes, in the order they are generated by NodeIterator.
//     Let `node[k]` denote the node with index `k` in this sequence.
//   * Descendents-First-Relationship means that for any `node[k]`, all its descendents
//     have indices strictly smaller than k in the iterator's sequence.
...
func (mt *MTrie) GetNodes() []*node.Node {
	var nodes []*node.Node
	itr := mt.NewNodeIterator()
	for n, ok := itr.Next(); ok; n, ok = itr.Next() {
