...
//     The caller must acquire the [storage.LockInsertMyReceipt] and hold it until the database write has been committed.
//
// Expected error returns during normal operations:
...
func IndexStateCommitment(lctx lockctx.Proof, rw storage.ReaderBatchWriter, blockID flow.Identifier, commit flow.StateCommitment) error {
	if !lctx.HoldsLock(storage.LockInsertMyReceipt) {
		return fmt.Errorf("cannot index state commitment without holding lock %s", storage.LockInsertMyReceipt)
	}
