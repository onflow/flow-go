...
func (_m *Transactions) ByFingerprint(fingerprint model.Fingerprint) (*flow.Transaction, error) {
	ret := _m.Called(fingerprint)

	var r0 *flow.Transaction
	if rf, ok := ret.Get(0).(func(model.Fingerprint) *flow.Transaction); ok {
		r0 = rf(fingerprint)
	} else {
		if ret.Get(0) != nil {
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(model.Fingerprint) error); ok {
		r1 = rf(fingerprint)
	} else {
		r1 = ret.Error(1)
	}
