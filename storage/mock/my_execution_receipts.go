...
// BatchStoreMyReceipt provides a mock function with given fields: lctx, receipt, batch
func (_m *MyExecutionReceipts) BatchStoreMyReceipt(lctx lockctx.Proof, receipt *flow.ExecutionReceipt, batch storage.ReaderBatchWriter) error {
	ret := _m.Called(lctx, receipt, batch)
	...
	var r0 error
	if rf, ok := ret.Get(0).(func(lockctx.Proof, *flow.ExecutionReceipt, storage.ReaderBatchWriter) error); ok {
		r0 = rf(lctx, receipt, batch)
	} else {
		r0 = ret.Error(0)
	}
