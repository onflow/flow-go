...
func StorageLayer(t testing.TB, db *badger.DB) (
	*storage.Headers,
	*storage.Guarantees,
	*storage.Seals,
	*storage.Index,
	*storage.Payloads,
	*storage.Blocks,
	*storage.QuorumCertificates,
	*storage.EpochSetups,
	*storage.EpochCommits,
	*storage.EpochStatuses,
	*storage.ExecutionResults,
) {
	...
	return headers, guarantees, seals, index, payloads, blocks, qcs, setups, commits, statuses, results
}
...
func RunWithStorageLayer(t testing.TB, f func(*badger.DB, *storage.Headers, *storage.Guarantees, *storage.Seals, *storage.Index, *storage.Payloads, *storage.Blocks, *storage.EpochSetups, *storage.EpochCommits, *storage.EpochStatuses)) {
	unittest.RunWithBadgerDB(t, func(db *badger.DB) {
		headers, guarantees, seals, index, payloads, blocks, _, setups, commits, statuses, _ := StorageLayer(t, db)
