...
// IsValidMachineAccountConfig returns true if the machine account file exists at the default path and
// if the `access-address` and `qc-contract-address` is set else returns false.
func IsValidMachineAccountConfig(node *FlowNodeBuilder, accessAddress, contractAddress string) bool {

	// check if node machine account info file exists
	machineAccountInfoPath := filepath.Join(node.BaseConfig.BootstrapDir, fmt.Sprintf(bootstrap.PathNodeMachineAccountInfoPriv, node.Me.NodeID()))
	exists := io.FileExists(machineAccountInfoPath)

	if accessAddress == "" || contractAddress == "" || !exists {
