...
func (fnb *FlowNodeBuilder) baseFlags() {
	...
	// nclusters flag is deprecated, as it will be read from the root-nclusters.json file generated by bootstrap
	var deprecated uint
	fnb.flags.UintVar(&deprecated, "nclusters", 2, "(deprecated) number of collection node clusters")
...
func (fnb *FlowNodeBuilder) enqueueNetworkInit() {
	fnb.Component("network", func(builder *FlowNodeBuilder) (module.ReadyDoneAware, error) {
		...
		net, err := libp2p.NewNetwork(fnb.Logger, codec, participants, fnb.Me, fnb.Middleware, 10e2, topology, fnb.Metrics.Network)
		if err != nil {
...
// initNClusters is only needed pre-Epoch
func (fnb *FlowNodeBuilder) initNClusters() {
	data, err := io.ReadFile(filepath.Join(fnb.BaseConfig.BootstrapDir, bootstrap.PathNClusters))
	fnb.MustNot(err).Msg("could not find nclusters file")

	var nclusters bootstrap.NClusters
	err = json.Unmarshal(data, &nclusters)
	fnb.MustNot(err).Msg("could not parse nclusters file")

	fnb.NClusters = nclusters.NClusters
}
...
func (fnb *FlowNodeBuilder) initState() {
	state, err := protocol.NewState(
		...
		fnb.Storage.Blocks,
		protocol.SetClusters(fnb.NClusters),
	)
