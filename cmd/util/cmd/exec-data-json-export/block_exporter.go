...
func ExportBlocks(blockID flow.Identifier, dbPath string, outputPath string) (flow.StateCommitment, error) {
	...
	if err != nil {
		return flow.EmptyStateCommitment, fmt.Errorf("could not create block output file %w", err)
	}
	defer fi.Close()
	...
	for {
		...
		if err != nil {
			return flow.EmptyStateCommitment, fmt.Errorf("could not create a json obj for a block: %w", err)
		}
		_, err = blockWriter.WriteString(string(jsonData) + "\n")
		if err != nil {
			return flow.EmptyStateCommitment, fmt.Errorf("could not write block json to the file: %w", err)
		}
		blockWriter.Flush()
	...
	if err != nil {
		return flow.EmptyStateCommitment, fmt.Errorf("could not find state commitment for this block: %w", err)
	}
	return state, nil
}
