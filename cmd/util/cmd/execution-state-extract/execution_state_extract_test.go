...
func TestExtractExecutionState(t *testing.T) {
	...
	t.Run("empty WAL doesn't find anything", func(t *testing.T) {
		withDirs(t, func(datadir, execdir, outdir string) {
			err := extractExecutionState(execdir, unittest.StateCommitmentFixture(), outdir, zerolog.Nop(), nil)
			require.Error(t, err)
		})
	})
	...
	t.Run("happy path", func(t *testing.T) {
		...
		t.Skip("Skipping for one-time migration")
		...
		//
		//			storage, err := completeLedger.NewLedger(outdir, 1000, metr, zerolog.Nop(), nil)
		//			//storage, err := oldLedger.NewMTrieStorage(outdir, 1000, metr, nil)
		//			require.NoError(t, err)
		//
		//			data := keysValuesByCommit[string(stateCommitment)]
		//
		//			registerIDs := make([]flow.RegisterID, 0, len(data))
		//			for _, v := range data {
		//				registerIDs = append(registerIDs, v.key)
		//			}
		//
		//			keys := state.RegisterIDSToKeys(registerIDs)
		//
		//			ledger.NewQuery(stateCommitment, keys)
		//			storage.Get()
		//
