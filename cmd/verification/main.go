...
func main() {
	verificationBuilder := NewVerificationNodeBuilder(cmd.FlowNode(flow.RoleVerification.String()))
	verificationBuilder.LoadFlags()

	// TODO: rename err2 to err after remove original code from main() - currently getting re-declaration error if leave it as err
	if err2 := verificationBuilder.FlowNodeBuilder.Initialize(); err2 != nil {
		verificationBuilder.FlowNodeBuilder.Logger.Fatal().Err(err2).Send()
	}

	verificationBuilder.LoadComponentsAndModules()

	// TODO: rename err2 to err after remove original code from main() - currently getting re-declaration error if leave it as err
	node, err2 := verificationBuilder.FlowNodeBuilder.Build()
	if err2 != nil {
		verificationBuilder.FlowNodeBuilder.Logger.Fatal().Err(err2).Send()
	}
	node.Run()

	// original BELOW until end of main()
	var (
		followerState protocol.MutableState
		err           error
		receiptLimit  uint // size of execution-receipt/result related memory pools.
