...
func main() {
	...
	cmd.FlowNode("consensus").
		ExtraFlags(func(flags *pflag.FlagSet) {
			...
			flags.DurationVar(&hotstuffTimeout, "hotstuff-timeout", time.Minute, "the initial timeout for the hotstuff pacemaker")
		}).
		...
		Component("consensus components", func(node *cmd.FlowNodeBuilder) (module.ReadyDoneAware, error) {
			...
			comp, err := compliance.New(node.Logger, node.Network, node.Me, node.State, headersDB, payloadsDB, cache, prov)
			if err != nil {
