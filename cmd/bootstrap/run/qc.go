...
func createValidators(ps *protoBadger.State, signerData SignerData, block *flow.Block) ([]*validator.Validator, []*signature.RandomBeaconAwareSigProvider, error) {
	...
	for i, signer := range signerData.Signers {
		...
		// create signer
		s, err := NewRandomBeaconSigProvider(ps, signerData.DKGState, signer.Identity(), keys.StakingKey, signer.RandomBeaconPrivKey)
		if err != nil {
			return nil, nil, err
		}
		signers[i] = s
		...
		// create view state
		vs, err := viewstate.New(ps, signerData.DKGState, signer.NodeID, filter.HasRole(flow.RoleConsensus))
		if err != nil {
			return nil, nil, err
		}
