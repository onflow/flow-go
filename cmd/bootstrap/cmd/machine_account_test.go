...
func TestMachineAccountHappyPath(t *testing.T) {
	var machineAccountHappyPath = `^read machine account private key json` +
		...
		`wrote file /tmp/%s/private-root-information/private-node-info_\S+/node-machine-account-info.priv.json`

	dirName := strconv.FormatInt(time.Now().UnixNano(), 10)
	regex := regexp.MustCompile(fmt.Sprintf(machineAccountHappyPath, dirName))
