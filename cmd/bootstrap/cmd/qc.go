...
func constructRootQC(block *flow.Block, votes []*model.Vote, allNodes []bootstrap.NodeInfo, internalNodes []bootstrap.NodeInfoPriv, randomBeaconData dkg.ThresholdKeySet) *flow.QuorumCertificate {
...
func constructRootVotes(block *flow.Block, allNodes []bootstrap.NodeInfo, internalNodes []bootstrap.NodeInfoPriv, randomBeaconData dkg.ThresholdKeySet) {
	participantData, err := run.GenerateQCParticipantData(allNodes, internalNodes, randomBeaconData)
	if err != nil {
		log.Fatal().Err(err).Msg("failed to generate QC participant data")
	}
