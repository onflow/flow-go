...
func genGenesisExecutionState(serviceAccountKey *flow.AccountPublicKey, genesisTokenSupply uint64, chain flow.Chain) flow.StateCommitment {
	...
	stateCommitment, err := run.GenerateExecutionState(dbPath, *serviceAccountKey, genesisTokenSupply, chain)
	if err != nil {
