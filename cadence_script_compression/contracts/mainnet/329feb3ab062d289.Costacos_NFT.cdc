...
pub contract Costacos_NFT: NonFungibleToken {
    ...
    // Costacos_NFT Events
    //
    // Emitted when the Costacos_NFT contract is created
    ...
    // totalSupply
    // The total number of Costacos_NFT that have been minted
    //
    ...
    // A Series is special resource type that contains functions to mint Costacos_NFT NFTs, 
	...
	pub resource Series {
        ...
        pub fun addNftSet(
            ...
            Costacos_NFT.setData[setId] = newNFTSet
        ...
        pub fun updateSeriesMetadata(metadata: {String: String}) {
            ...
            Costacos_NFT.seriesData[self.seriesId] = newSeriesMetadata
        ...
        pub fun updateSetMetadata(
            ...
            Costacos_NFT.setData[setId] = newSetMetadata
	    ...
	    pub fun mintCostacos_NFT(
            ...
            pre {
                ...
                self.numberEditionsMintedPerSet[setId]! <= Costacos_NFT.getSetMaxEditions(setId: setId)!:
			...
			recipient.deposit(token: <-create Costacos_NFT.NFT(
                tokenId: tokenId,
                setId: setId,
    ...
    pub resource Admin {
        ...
        pub fun addSeries(seriesId: UInt32, metadata: {String: String}) {
            pre {
                Costacos_NFT.series[seriesId] == nil:
            ...
            Costacos_NFT.series[seriesId] <-! newSeries
        }
    ...
    // to allow others to deposit Costacos_NFT into their Collection. It also allows for reading
    // the details of Costacos_NFT in the Collection.
    pub resource interface Costacos_NFTCollectionPublic {
        ...
        pub fun borrowCostacos_NFT(id: UInt64): &Costacos_NFT.NFT? {
            ...
            post {
                (result == nil) || (result?.id == id):
                    "Cannot borrow Costacos_NFT reference: The ID of the returned reference is incorrect"
            }
        }
    }
    ...
    pub resource Collection: Costacos_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
        ...
        //
        pub fun deposit(token: @NonFungibleToken.NFT) {
            let token <- token as! @Costacos_NFT.NFT
        ...
        // borrowCostacos_NFT
        // Gets a reference to an NFT in the collection as a Costacos_NFT,
    ...
    // fetch
    // Get a reference to a Costacos_NFT from an account's Collection, if available.
    // If an account does not have a Costacos_NFT.Collection, panic.
    ...
    //
    pub fun fetch(_ from: Address, id: UInt64): &Costacos_NFT.NFT? {
        let collection = getAccount(from)
            .getCapability(Costacos_NFT.CollectionPublicPath)
            .borrow<&Costacos_NFT.Collection{Costacos_NFT.Costacos_NFTCollectionPublic}>()
            ?? panic("Couldn't get collection")
        // We trust Costacos_NFT.Collection.borrowCostacos_NFT to get the correct id
        // (it checks it before returning it).
        return collection.borrowCostacos_NFT(id: id)
    }

    // getAllSeries returns all the sets
    //
    // Returns: An array of all the series that have been created
    pub fun getAllSeries(): [Costacos_NFT.SeriesData] {
        return Costacos_NFT.seriesData.values
    }

    // getAllSets returns all the sets
    //
    // Returns: An array of all the sets that have been created
    pub fun getAllSets(): [Costacos_NFT.NFTSetData] {
        return Costacos_NFT.setData.values
    }
    ...
    pub fun getSeriesMetadata(seriesId: UInt32): {String: String}? {
        return Costacos_NFT.seriesData[seriesId]?.getMetadata()
    }
    ...
    pub fun getSetMaxEditions(setId: UInt32): UInt32? {
        return Costacos_NFT.setData[setId]?.maxEditions
    }
    ...
    pub fun getSetMetadata(setId: UInt32): {String: String}? {
        return Costacos_NFT.setData[setId]?.getMetadata()
    }
    ...
    pub fun getSetSeriesId(setId: UInt32): UInt32? {
        return Costacos_NFT.setData[setId]?.seriesId
    }
    ...
    pub fun getIpfsMetadataHashByNftEdition(setId: UInt32, editionNum: UInt32): String? {
        ...
        if let set = Costacos_NFT.setData[setId] {
	...
	init() {
        // Set named paths
        self.CollectionStoragePath = /storage/Costacos_NFTCollection
        self.CollectionPublicPath = /public/Costacos_NFTCollection
        self.AdminStoragePath = /storage/Costacos_NFTAdmin
        self.AdminPrivatePath = /private/Costacos_NFTAdminUpgrade
        ...
        self.account.link<&Costacos_NFT.Admin>(
