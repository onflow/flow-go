...
//
pub contract SportsIconNFTStorefront {
    ...
    pub struct ListingDetails {
        ...
        // The start time allowing NFT purchases of this listing.
        pub let startTime: UFix64
        // The end time disallowing future NFT purchases of this listing.
        ...
        init (
            ...
            assert(self.startTime < self.endTime, message: "Start time must be before end time")
            assert(self.endTime > getCurrentBlock().timestamp, message: "End time can not be set to a past time")
        }
    }
    ...
    pub resource Listing: ListingPublic {
        ...
        pub fun purchase(payment: @FungibleToken.Vault): @NonFungibleToken.NFT {
            pre {
                ...
                getCurrentBlock().timestamp >= self.details.startTime: "Sale has not started for this listing"
                getCurrentBlock().timestamp < self.details.endTime: "Sale has expired"
            }
            ...
            emit ListingCompleted(
                ...
                priceTransacted: self.details.salePrice,
    ...
    pub resource Storefront : StorefrontManager, StorefrontPublic {
        ...
        // cleanup
        // Remove an listing *if* it has been purchased or if it has passed its expiration time.
        ...
        pub fun cleanup(listingResourceID: UInt64) {
            ...
            assert(listingDetails.purchased == true || getCurrentBlock().timestamp > listingDetails.endTime, message: "listing is still active, only admin can remove")
            destroy listing
        }
    ...
    init () {
        self.StorefrontStoragePath = /storage/SportsIconCustomNFTStorefront
        self.StorefrontPublicPath = /public/SportsIconCustomNFTStorefront
