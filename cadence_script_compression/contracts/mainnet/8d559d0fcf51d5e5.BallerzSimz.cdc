...
import BounceToken from 0x8d559d0fcf51d5e5
...
pub contract BallerzSimz {
    ...
    ******************/
    ...
    access(contract) let activeBallerIdz: {UInt64: Bool}
    ...
    *****************/
    ...
    pub event SimCompleted(
        ...
        winnerScores: [UInt16], loserScores: [UInt16], winningPayout: UFix64, bonuses: {String: String}
    )
    pub event SimCancelled(simId: UInt64, teamIds: [UInt64])
    ...
    pub event ContractPausedSet(value: Bool)
    ...
    *****************/
    ...
    pub resource interface BallerzSimzAdmin {
        ...
        pub fun cancelSim(simId: UInt64)
    ...
    pub resource interface BallerzSimzConfigAdmin {
        ...
        pub fun setContractPaused(value: Bool)
        pub fun setAcceptedBallerzSetIdz(newSetIdz: [UInt64])

        pub fun getCurrentEntryFees(): [UFix64]
        pub fun getEntryFeeToFeePercentageMap(): {UFix64: UFix64}
        pub fun getTeamSizes(): [UInt8]
        pub fun getContractPaused(): Bool
        pub fun getAcceptedBallerzSetIdz(): [UInt64]
    }
    ...
    pub resource ActiveSim {
        ...
        pub let feePercentage: UFix64
        access(contract) let teams: @{UInt64: BallerzTeam}
        access(contract) let bonuses: {String: String}

        init(simId: UInt64, team1: @BallerzTeam, team2: @BallerzTeam, entryAmount: UFix64, feePercentage: UFix64, bonuses: {String: String}) {
            pre {
    ...
    pub resource BallerzTeam {
        ...
        init(teamSize: UInt8, privateTeam: Bool, teamName: String, startingBaller: &Gaia.NFT, entryTokens: @FungibleToken.Vault) {
            ...
            self.joinTeam(baller: startingBaller, entryTokens: <-entryTokens, requiredEntryTokenType: Type<@BounceToken.Vault>())
        ...
        access(contract) fun joinTeam(baller: &Gaia.NFT, entryTokens: @FungibleToken.Vault, requiredEntryTokenType: Type) {
            pre {
                self.ballerIdz.length < Int(self.teamSize) : "This team is full"
                !self.ballerIdz.contains(baller.id) : "Baller already on team"
                !BallerzSimz.getActiveBallerIdz().containsKey(baller.id) : "Baller already active"
                self.startedSim == false : "Sim already started with team"
                entryTokens.balance == self.entryFee : "Incorrect entry fee"
                entryTokens.isInstance(requiredEntryTokenType) : "Incorrect entry token type"
                BallerzSimz.getAcceptedBallerzSetIdz().contains(baller.data.setID) : "Not a Baller"
            }
            ...
            self.ownerReceivers.insert(
                key: baller.id,
                ballerOwner.getCapability<&{FungibleToken.Receiver}>(BounceToken.bounceTokenReceiverPath)
            )
            // store entry tokens
        ...
        access(contract) fun stopStartedSimForTeam() {
            pre {
                self.startedSim == true : "Sim not started for team"
            }
        ...
        *****************/
        ...
        pub fun createTeam(baller: &Gaia.NFT, entryTokens: @FungibleToken.Vault, teamSize: UInt8, privateTeam: Bool, teamName: String): UInt64 {
            pre {
                ...
                !BallerzSimz.getContractPaused() : "Contract paused"
            }
        ...
        pub fun completeSim(
            ...
            emit SimCompleted(
                ...
                winnerScores: winnerScores, loserScores: loserScores, winningPayout: winningOwnerPayoutAmount, bonuses: sim.bonuses
            )
    ...
    pub resource BallerzSimzConfig : BallerzSimzConfigAdmin {
        ...
        access(self) var contractPaused: Bool
        access(self) var acceptedBallerzSetIdz: [UInt64]
        ...
        pub fun setAcceptedBallerzSetIdz(newSetIdz: [UInt64]) {
            pre {
                newSetIdz.length > 0 : "Must have at least one set id"
            }
    ...
    pub fun getActiveBallerIdz(): {UInt64: Bool} {
    ...
    init() {
        ...
        self.account.save(<- BounceToken.createEmptyVault(), to: /storage/BallerzOwnerEscrowVault)
