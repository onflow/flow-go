...
docker-build-util:
	docker build -f cmd/Dockerfile --ssh default --ssh default --build-arg TARGET=util --target production \
...
docker-build-remove-execution-fork:
	docker build -f cmd/Dockerfile --ssh default --ssh default --build-arg TARGET=util/cmd/remove-execution-fork --target production \
