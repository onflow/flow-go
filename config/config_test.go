...
func TestBindPFlags(t *testing.T) {
	t.Run("should override config values when any flag is set", func(t *testing.T) {
		...
		require.Equal(t, c.NetworkConfig.GossipsubScorePenalties.Graft, float64(-10))
		require.Equal(t, c.NetworkConfig.GossipsubScorePenalties.Prune, float64(-5))
		require.Equal(t, c.NetworkConfig.GossipsubScorePenalties.IHave, float64(-2))
		require.Equal(t, c.NetworkConfig.GossipsubScorePenalties.IWant, float64(-.9))
		require.Equal(t, c.NetworkConfig.GossipsubScorePenalties.Publish, float64(-.1))
	})
