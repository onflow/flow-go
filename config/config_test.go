...
func TestBindPFlags(t *testing.T) {
	t.Run("should override config values when any flag is set", func(t *testing.T) {
		...
		err = flags.Set("gossipsub-score-penalty-graft", "-10")
		require.NoError(t, err)
		err = flags.Set("gossipsub-score-penalty-prune", "-5")
		require.NoError(t, err)
		err = flags.Set("gossipsub-score-penalty-ihave", "-2")
		require.NoError(t, err)
		err = flags.Set("gossipsub-score-penalty-iwant", "-.9")
		require.NoError(t, err)
		err = flags.Set("gossipsub-score-penalty-publish", "-.1")
		require.NoError(t, err)
		...
		require.Equal(t, c.NetworkConfig.GossipSub.ScoringParameters.GossipsubScorePenalties.Graft, float64(-10))
		require.Equal(t, c.NetworkConfig.GossipSub.ScoringParameters.GossipsubScorePenalties.Prune, float64(-5))
		require.Equal(t, c.NetworkConfig.GossipSub.ScoringParameters.GossipsubScorePenalties.IHave, float64(-2))
		require.Equal(t, c.NetworkConfig.GossipSub.ScoringParameters.GossipsubScorePenalties.IWant, float64(-.9))
		require.Equal(t, c.NetworkConfig.GossipSub.ScoringParameters.GossipsubScorePenalties.Publish, float64(-.1))
	})
