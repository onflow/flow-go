...
* `EventHandler` orchestrates all HotStuff components and implements the [HotStuff's state machine](https://github.com/dapperlabs/flow-go/blob/master/docs/StateMachine.png).
...
* `model` contains the HotStuff data models, including block proposal, QC, etc
* `notifications`: All relevant events within the HotStuff logic are exported though a notification system. While the notifications are _not_ used HotStuff-internally, they notify other components within the same node of relevant progress and are used for collecting consensus metrics.
* `pacemaker` contains the implementation of Flow's basic PaceMaker, as described above.
* `signature` contains integration of Flow's cryptography. 
* `validator` holds the logic for validating the HotStuff-relevant aspects of blocks, QCs, and votes
* `viewstate` maintains the list of all authorized network members and their respective stake on a per-block basis.
* `voteaggregator` caches votes on a per-block basis and builds a QC if enough votes have been accumulated.
* `voter` tracks the view of the latest vote and determines whether or not to vote for a block
