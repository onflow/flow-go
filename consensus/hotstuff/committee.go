...
// commonCommittee defines methods shared between VoterCommittee and ProposerCommittee.
//
...
// from that node. For these purposes we use the ProposerCommittee and *ByBlock methods.
//
type commonCommittee interface {
	...
	DKG(view uint64) (protocol.DKG, error)
}
...
// * we can query by view
// * we don't need the full block ancestry prior to validating messages
//
// CAUTION: DO NOT use this for validating block proposals.
...
// ProposerCommittee defines the consensus committee for the purposes of validating proposals.
// The proposer committee reflects block-to-block changes in the identity table to support
// immediately rejecting proposals from nodes after they are ejected.
//
...
type ProposerCommittee interface {
	commonCommittee
	...
	// ERROR conditions:
	//  * model.InvalidSignerError if participantID does NOT correspond to an authorized HotStuff participant at the specified block.
	IdentityByBlock(view uint64, participantID flow.Identifier) (*flow.Identity, error)
}
