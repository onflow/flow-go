...
func EnsureVoteForBlock(vote *model.Vote, block *model.Block) error {
	if vote.View != block.View {
		return fmt.Errorf("vote %v has view %d while block's view is %d: %w ", vote.Hash(), vote.View, block.View, VoteForIncompatibleViewError)
	}
	if vote.BlockID != block.BlockID {
		return fmt.Errorf("expecting only votes for block %v, but vote %v is for block %v: %w ", block.BlockID, vote.Hash(), vote.BlockID, VoteForIncompatibleBlockError)
	}
	return nil
}
