...
func TestConstructor(t *testing.T) {
	c, err := NewConfig(2200*time.Millisecond, 1200*time.Millisecond, 2000*time.Millisecond, 1.5, 0.85, time.Second)
	...
	_, err = NewConfig(800*time.Millisecond, 1200*time.Millisecond, 2000*time.Millisecond, 1.5, 0.85, time.Second)
	...
	c, err = NewConfig(2200*time.Millisecond, 1200*time.Millisecond, 1000*time.Millisecond, 1.5, 0.85, time.Second)
	require.True(t, model.IsConfigurationError(err))

	// should not allow timeoutIncrease to be 1.0 or smaller
	c, err = NewConfig(2200*time.Millisecond, 1200*time.Millisecond, 2000*time.Millisecond, 1.0, 0.85, time.Second)
	require.True(t, model.IsConfigurationError(err))

	// should not allow timeoutDecrease to be zero or 1.0
	c, err = NewConfig(2200*time.Millisecond, 1200*time.Millisecond, 2000*time.Millisecond, 1.5, 0, time.Second)
	require.True(t, model.IsConfigurationError(err))
	c, err = NewConfig(2200*time.Millisecond, 1200*time.Millisecond, 2000*time.Millisecond, 1.5, 1, time.Second)
	require.True(t, model.IsConfigurationError(err))

	// should not allow blockRateDelay to be zero negative
	c, err = NewConfig(2200*time.Millisecond, 1200*time.Millisecond, 2000*time.Millisecond, 1.5, 0.85, -1*time.Nanosecond)
	require.True(t, model.IsConfigurationError(err))
}
