...
func TestTriggerOnlyOneCheckpointerIsRunning(t *testing.T) {
	...
	for i := activeSegmentNum + 2; i < activeSegmentNum+20; i++ {
	...
	time.Sleep(1 * time.Millisecond)
	runner.AssertCalled(t, "RunCheckpoint", ctx, queue.Tries(), 10)
	runner.AssertExpectations(t) // in total was only called once
}
