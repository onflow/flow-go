...
func Test_WAL(t *testing.T) {
	...
	unittest.RunWithTempDir(t, func(dir string) {
		...
		compactor2, err := complete.NewCompactor(led2, diskWal2, zerolog.Nop(), uint(size), checkpointDistance, checkpointsToKeep, noopTrigger)
		require.NoError(t, err)
...
func TestLedgerFunctionality(t *testing.T) {
	...
	for e := 0; e < experimentRep; e++ {
		...
		unittest.RunWithTempDir(t, func(dbDir string) {
			...
			assert.NoError(t, err)
			noopTrigger := make(chan interface{})
			compactor, err := complete.NewCompactor(led, diskWal, zerolog.Nop(), uint(activeTries), checkpointDistance, checkpointsToKeep, noopTrigger)
			require.NoError(t, err)
			<-compactor.Ready()
	...
	t.Run("noop migration", func(t *testing.T) {
		...
		unittest.RunWithTempDir(t, func(dbDir string) {
			unittest.RunWithTempDir(t, func(dir2 string) {
				...
				require.NoError(t, err)
				noopTrigger := make(chan interface{})
				compactor, err := complete.NewCompactor(led, diskWal, zerolog.Nop(), capacity, checkpointDistance, checkpointsToKeep, noopTrigger)
				require.NoError(t, err)
				<-compactor.Ready()
				...
				require.NoError(t, err)
				compactor2, err := complete.NewCompactor(led2, diskWal2, zerolog.Nop(), capacity, checkpointDistance, checkpointsToKeep, noopTrigger)
				require.NoError(t, err)
				<-compactor2.Ready()
...
func TestWALUpdateFailuresBubbleUp(t *testing.T) {
	unittest.RunWithTempDir(t, func(dir string) {
		...
		noopTrigger := make(chan interface{})
		compactor, err := complete.NewCompactor(led, w, zerolog.Nop(), capacity, checkpointDistance, checkpointsToKeep, noopTrigger)
		require.NoError(t, err)
