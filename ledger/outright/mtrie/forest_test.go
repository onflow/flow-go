...
import (
	...
	"github.com/dapperlabs/flow-go/ledger/ptrie"
...
func TestTrieOperations(t *testing.T) {
	...
	updatedTrie, err := trie.NewTrieWithUpdatedRegisters(nt, [][]byte{k1}, [][]byte{k1}, [][]byte{v1})
	require.NoError(t, err)
...
func TestRandomUpdateReadProof(t *testing.T) {
	...
	metricsCollector := &metrics.NoopCollector{}
	fStore, err := NewMForest(keyByteSize, dir, 5, metricsCollector, nil, common.KeyToPath)
	require.NoError(t, err)
	...
	for e := 0; e < rep; e++ {
		paths := common.GetRandomPathsRandN(maxNumPathsPerStep, pathByteSize)
		keys := common.GetRandomValues(len(paths), maxByteSize)
		values := common.GetRandomValues(len(paths), maxByteSize)
		...
		psmt, err := ptrie.NewPSMT(testTrie.RootHash(), keyByteSize, proofKeys, proofValues, proofToGo, common.KeyToPath)
