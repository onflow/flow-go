...
func VerifyProof(p *ledger.Proof, path ledger.Path, payload ledger.Payload, expectedRootHash []byte, expectedKeySize int) bool {
	...
	computed := ComputeCompactValue(path, &payload, leafHeight) // we first compute the hash of the fully-expanded leaf (at height 0)
...
func VerifyBatchProof(bp *ledger.BatchProof, paths []ledger.Path, payloads []ledger.Payload, expectedRootHash []byte, expectedKeySize int) bool {
	for i, p := range bp.Proofs {
		// any invalid proof
		if !VerifyProof(p, paths[i], payloads[i], expectedRootHash, expectedKeySize) {
			return false
		}
	}
	return true
