...
func New(log zerolog.Logger,
	...
	// TODO: Call registerConduits
	// TODO: Pass conduits to handler
	eng := &Engine{
		log:     log,
		unit:    engine.NewUnit(),
		handler: NewHandler(log, state),
		server:  grpc.NewServer(),
		config:  config,
	}

	protobuf.RegisterGhostSenderServer(eng.server, eng.handler)

	return eng
}

func registerConduits(net module.Network, eng network.Engine) {
