...
func NewTxTracker(
	...
	sleepAfterOp time.Duration) (*TxTracker, error) {
	...
	for i := 0; i < numberOfWorkers; i++ {
		...
		go statusWorker(log, i, txt.txs, txt.done, fclient, sleepAfterOp)
	}
	return txt, nil
}
...
func statusWorker(log zerolog.Logger, workerID int, txs chan *txInFlight, done <-chan bool, fclient *client.Client, sleepAfterOp time.Duration) {
