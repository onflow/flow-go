...
func (suite *VerificationTestSuite) TestVerificationNodeHappyPath() {
	...
	suite.T().Logf("blockA generated, height: %v ID: %v", blockA.Header.Height, blockA.Header.ID())
	...
	suite.T().Logf("receipt for blockA generated: result ID: %x", resultID)
...
func (suite *VerificationTestSuite) TestSealingAndVerificationHappyPath() {
	...
	suite.T().Logf("receipt for blockB generated by execution node-1: %x result ID: %x", suite.exe1ID, receiptB1.ExecutionResult.ID())
	receiptB2 := suite.ReceiptState.WaitForReceiptFrom(suite.T(), blockB.Header.ID(), suite.exe2ID)
	suite.T().Logf("receipt for blockB generated by execution node-2: %x result ID: %x", suite.exe2ID, receiptB2.ExecutionResult.ID())
...
func (suite *VerificationTestSuite) TestSystemChunkIDsShouldBeDifferent() {
	...
	suite.T().Logf("blockB generated, height: %v ID: %v", blockB.Header.Height, blockB.Header.ID())

	// waits for execution receipt for blockA from execution node, called receiptA.
	receiptA := suite.ReceiptState.WaitForReceiptFrom(suite.T(), blockA.Header.ID(), suite.exe1ID)
	resultAId := receiptA.ExecutionResult.ID()
	suite.T().Logf("receipt for blockA generated: result ID: %x", resultAId)
	...
	suite.T().Logf("receipt for blockB generated: result ID: %x", resultBId)
