...
func (w *WintermuteTestSuite) TestWintermuteAttack() {
	...
	receiptB1 := w.ReceiptState.WaitForReceiptFrom(w.T(), victimBlock.Header.ID(), w.exe1Id)
	w.T().Logf("receipt for victim block generated by execution node-1: %x result ID: %x\n", w.exe1Id, receiptB1.ExecutionResult.ID())
	receiptB2 := w.ReceiptState.WaitForReceiptFrom(w.T(), victimBlock.Header.ID(), w.exe2Id)
	...
	for i := 0; i < len(corruptedResult.Chunks); i++ {
		w.ApprovalState.WaitForTotalApprovalsFrom(w.T(),
			flow.IdentifierList{w.ver1Id, w.ver2Id, w.ver3Id},
			corruptedResult.ID(),
			uint64(i),
			2)
	}
