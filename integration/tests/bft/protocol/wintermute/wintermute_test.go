...
func (w *WintermuteTestSuite) TestWintermuteAttack() {
	...
	receiptB1 := w.ReceiptState.WaitForReceiptFrom(w.T(), victimBlock.Hash(), w.corruptedEN1Id)
	w.T().Logf("receipt for victim block generated by execution node-1: %x result ID: %x\n", w.corruptedEN1Id, receiptB1.ExecutionResult.Hash())
	receiptB2 := w.ReceiptState.WaitForReceiptFrom(w.T(), victimBlock.Hash(), w.corruptedEN2Id)
	w.T().Logf("receipt for victim block generated by execution node-2: %x result ID: %x\n", w.corruptedEN2Id, receiptB2.ExecutionResult.Hash())

	// makes sure corrupted execution nodes generated the corrupted result for this victim block.
	require.Equal(w.T(), receiptB1.ExecutionResult.Hash(), corruptedResult.Hash())
	require.Equal(w.T(), receiptB2.ExecutionResult.Hash(), corruptedResult.Hash())
	...
	require.True(w.T(), ok)
	require.Equal(w.T(), blockByHeight.Hash(), victimBlock.Hash())
}
...
func (w *WintermuteTestSuite) waitForExecutionResultCorruption() *flow.ExecutionResult {
	...
	require.Eventually(w.T(), func() bool {
		...
		w.T().Logf("wintermute orchesterator conducted corruption, original result: %x, corrupted result: %x", original.Hash(), corrupted.Hash())
		return true
