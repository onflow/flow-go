...
import (
	...
	util2 "github.com/onflow/flow-go/state/protocol/util"
...
func (suite *MutatorSuite) SetupTest() {
	...
	suite.protoState, err = pbadger.NewFollowerState(state, index, conPayloads, tracer, consumer, util2.MockBlockTimer())
	require.NoError(suite.T(), err)
}
