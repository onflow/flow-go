...
func (s *MutatorSuite) SetupTest() {
	...
	s.mutator = NewMutator(s.headersDB, s.resultsDB, s.setupsDB, s.commitsDB, s.stateMachine, s.params)
}
...
//	updater, err := s.mutator.(candidate.View, candidate.ParentID)
...
//}
//
//// TestMutatorApplyServiceEvents_InvalidEpochSetup tests that handleServiceEvents rejects invalid epoch setup event and sets
...
//
//		updates, err := s.mutator.handleServiceEvents(updater, []*flow.Seal{seal})
//		require.NoError(s.T(), err)
...
//}
//
//// TestMutatorApplyServiceEvents_InvalidEpochCommit tests that handleServiceEvents rejects invalid epoch commit event and sets
//// InvalidStateTransitionAttempted flag in protocol.ProtocolStateMachine.
...
//}
//
//// TestMutatorApplyServiceEventsSealsOrdered tests that handleServiceEvents processes seals in order of block height.
...
//	}))
//
//	updates, err := s.mutator.handleServiceEvents(updater, seals)
//	require.NoError(s.T(), err)
//	require.Empty(s.T(), updates)
...
//}
//
//// TestMutatorApplyServiceEventsTransitionToNextEpoch tests that handleServiceEvents transitions to the next epoch
...
//	dbUpdates, err := s.mutator.handleServiceEvents(updater, []*flow.Seal{})
//	require.NoError(s.T(), err)
...
//	_, err := s.mutator.handleServiceEvents(updater, []*flow.Seal{})
