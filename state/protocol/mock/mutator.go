...
// Bootstrap provides a mock function with given fields: block, result, seal
func (_m *Mutator) Bootstrap(block *flow.Block, result *flow.ExecutionResult, seal *flow.Seal) error {
	ret := _m.Called(block, result, seal)

	var r0 error
	if rf, ok := ret.Get(0).(func(*flow.Block, *flow.ExecutionResult, *flow.Seal) error); ok {
		r0 = rf(block, result, seal)
	} else {
		r0 = ret.Error(0)
	}
