...
func ProtocolState(t testing.TB, db *badger.DB, options ...func(*protocol.State)) *protocol.State {
	...
	headers, identities, _, seals, _, payloads, blocks := util.StorageLayer(t, db)
	proto, err := protocol.NewState(metrics, db, headers, identities, seals, payloads, blocks, options...)
