...
func RunWithBootstrapState(t testing.TB, rootSnapshot protocol.Snapshot, f func(*badger.DB, *pbadger.State)) {
	unittest.RunWithBadgerDB(t, func(db *badger.DB) {
		...
		state, err := pbadger.Bootstrap(metrics, db, all.Headers, all.Seals, all.Results, all.Blocks, all.Setups, all.EpochCommits, all.Statuses, all.VersionBeacons, rootSnapshot)
...
func RunWithFullProtocolState(t testing.TB, rootSnapshot protocol.Snapshot, f func(*badger.DB, *pbadger.MutableState)) {
	unittest.RunWithBadgerDB(t, func(db *badger.DB) {
		...
		fullState, err := pbadger.NewFullConsensusState(state, all.Index, all.Payloads, tracer, log, consumer, mockTimer, receiptValidator, sealValidator)
		require.NoError(t, err)
		f(db, fullState)
	})
}
...
func RunWithFullProtocolStateAndValidator(t testing.TB, rootSnapshot protocol.Snapshot, validator module.ReceiptValidator, f func(*badger.DB, *pbadger.MutableState)) {
	unittest.RunWithBadgerDB(t, func(db *badger.DB) {
		...
		sealValidator := MockSealValidator(all.Seals)
		mockTimer := MockBlockTimer()
		fullState, err := pbadger.NewFullConsensusState(state, all.Index, all.Payloads, tracer, log, consumer, mockTimer, validator, sealValidator)
		require.NoError(t, err)
		f(db, fullState)
	})
}
...
func RunWithFollowerProtocolState(t testing.TB, rootSnapshot protocol.Snapshot, f func(*badger.DB, *pbadger.FollowerState)) {
	unittest.RunWithBadgerDB(t, func(db *badger.DB) {
		...
		require.NoError(t, err)
		mockTimer := MockBlockTimer()
		followerState, err := pbadger.NewFollowerState(state, all.Index, all.Payloads, tracer, log, consumer, mockTimer)
		require.NoError(t, err)
		f(db, followerState)
	})
}
