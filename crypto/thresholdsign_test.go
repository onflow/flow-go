...
func TestThresholdSignature(t *testing.T) {
	t.Run("Steteful Threshold signature with simple key generation", testStatefulThresholdSignatureSimpleKeyGen)
	...
	t.Run("Steteless Threshold signature with simple key generation", testStatelessThresholdSignatureSimpleKeyGen)
...
func testStatefulThresholdSignatureSimpleKeyGen(t *testing.T) {
	...
	for threshold := 0; threshold < n; threshold++ {
		...
		require.NoError(t, err)
		ts.SetKeys(skShares[index], pkGroup, pkShares)
