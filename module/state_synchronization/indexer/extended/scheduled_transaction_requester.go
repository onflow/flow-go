...
func (r *ScheduledTransactionRequester) Fetch(
	...
	for batch := range slices.Chunk(lookupIDs, maxLookupBatchSize) {
		args, err := buildArgs(batch)
		if err != nil {
			return nil, fmt.Errorf("failed to build arguments: %w", err)
		}

		response, err := r.executor.ExecuteAtBlockHeight(ctx, r.script, args, lookupHeight)
		if err != nil {
...
func buildArgs(ids []uint64) ([][]byte, error) {
	args := make([][]byte, 0, len(ids))
	for _, id := range ids {
		encoded, err := jsoncdc.Encode(cadence.UInt64(id))
		if err != nil {
			return nil, fmt.Errorf("failed to encode scheduled tx ID: %w", err)
		}
...
// TransactionDataScript returns the Cadence script used for JIT scheduled transaction
