...
func (i *indexCoreTest) initIndexer() *indexCoreTest {
	...
	indexer, err := New(zerolog.New(os.Stdout), metrics.NewNoopCollector(), db, i.registers, i.headers, i.events, i.results, flow.Testnet.Chain(), derivedChainData, onCollection)
	require.NoError(i.t, err)
	i.indexer = indexer
	return i
}
...
func TestIndexerIntegration_StoreAndGet(t *testing.T) {
	...
	t.Run("Single Index Value Changes", func(t *testing.T) {
		pebbleStorage.RunWithRegistersStorageAtInitialHeights(t, 0, 0, func(registers *pebbleStorage.Registers) {
			index, err := New(logger, metrics, db, registers, nil, nil, nil, flow.Testnet.Chain(), derivedChainData, noopHandlerFunc)
			require.NoError(t, err)
