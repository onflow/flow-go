...
func (m *mockStateProvider) UpdateState(state optimistic_sync.State, pipeline *PipelineImpl) {
	m.state = state
...
func waitForStateUpdates(t *testing.T, updateChan <-chan optimistic_sync.State, expectedStates ...optimistic_sync.State) {
...
func waitNeverStateUpdate(t *testing.T, updateChan <-chan optimistic_sync.State) {
	done := make(chan struct{})
...
func createPipeline(t *testing.T) (*PipelineImpl, *osmock.Core, <-chan optimistic_sync.State, *mockStateProvider) {
