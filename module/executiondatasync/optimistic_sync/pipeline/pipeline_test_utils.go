...
// NewMockStateProvider initializes a mockStateProvider with the default state optimistic_sync..
...
func waitNeverStateUpdate(t *testing.T, updateChan <-chan optimistic_sync.State, errChan <-chan error) {
	done := make(chan struct{})
