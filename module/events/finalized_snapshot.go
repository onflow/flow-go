...
// consuming BlockFinalized events from HotStuff.
// NOTE: The protocol state guarantees that state.Final().Head() will be cached, however
// since the protocol state is shared among many components, there may be high contention
// on its cache.
// The FinalizedHeaderCache can be used in place of state.Final().Head() to avoid read
// contention with other components.
...
func NewFinalizedHeaderCache(log zerolog.Logger, state protocol.State, finalizationDistributor *pubsub.FinalizationDistributor) (*FinalizedHeaderCache, error) {
	cache := &FinalizedHeaderCache{
		...
		log:                       log.With().Str("sub_component", "finalized_snapshot_cache").Logger(),
