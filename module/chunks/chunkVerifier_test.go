...
func (s *ChunkVerifierTestSuite) TestEmptyCollection() {
	...
	emptyListHash, err := flow.EventsListMerkleHash(flow.EventsList{})
...
func GetBaselineVerifiableChunk(t *testing.T, script string, system bool) *verification.VerifiableChunkData {
	...
	EventsListMerkleHash, err := flow.EventsListMerkleHash(chunkEvents)
	require.NoError(t, err)
	...
	chunk := flow.Chunk{
		ChunkBody: flow.ChunkBody{
			...
			BlockID:         blockID,
			EventCollection: EventsListMerkleHash,
		},
		Index: 0,
	}
