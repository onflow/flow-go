...
type PendingBlockBuffer interface {
	Add(proposal flow.Slashable[flow.Block]) bool
...
type PendingClusterBlockBuffer interface {
	Add(originID flow.Identifier, proposal *cluster.Block) bool

	ByID(blockID flow.Identifier) (*cluster.Block, bool)

	ByParentID(parentID flow.Identifier) ([]*cluster.Block, bool)
