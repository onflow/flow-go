...
func (l *EpochLookup) EpochForView(view uint64) (epochCounter uint64, err error) {
	...
	currentEpoch, ok, err := l.getEpochInfo(view, epochs.Current())
	if err != nil {
	...
	if view < currentEpoch.firstView {
		previousEpoch, ok, err := l.getEpochInfo(view, epochs.Previous())
		if err != nil {
		...
		if ok {
			...
			if previousEpoch.firstView <= view && view <= previousEpoch.finalView {
	...
	if err != nil {
		return 0, fmt.Errorf("could not check if currentEpoch fallback triggered: %w", err)
	}

	return 0, fmt.Errorf("couldn't get currentEpoch for view %d", view)
}

// getEpochInfo determines whether the view is within the bounds of the given epoch.
...
func (l *EpochLookup) getEpochInfo(view uint64, epoch protocol.Epoch) (*epochInfo, bool, error) {
