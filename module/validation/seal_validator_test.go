...
func (s *SealValidationSuite) TestSealValid() {
	_, _, newBlock, _, _ := s.generateBasicTestFork()
...
func (s *SealValidationSuite) TestSeal_EnforceGap() {
	...
	newBlock := unittest.BlockWithParentFixture(b1.ToHeader())
...
func (s *SealValidationSuite) TestSealInvalidBlockID() {
	_, _, newBlock, _, seal := s.generateBasicTestFork()
	seal.BlockID = unittest.IdentifierFixture() // modify seal (seal pointer already included in newBlock's payload)
...
func (s *SealValidationSuite) TestSealInvalidAggregatedSigCount() {
	_, _, newBlock, _, seal := s.generateBasicTestFork()
	seal.AggregatedApprovalSigs = seal.AggregatedApprovalSigs[1:] // modify seal (seal pointer already included in newBlock's payload)
...
func (s *SealValidationSuite) TestSealEmergencySeal_VerificationRequire0ApprovalEmergencyNotTriggered() {
	instance, err := updatable_configs.NewSealingConfigs(2, 0, 3, false)
...
func (s *SealValidationSuite) TestSealEmergencySeal_VerificationRequire1ApprovalReceive1Approval() {
	instance, err := updatable_configs.NewSealingConfigs(2, 1, 3, false)
	require.NoError(s.T(), err)
	s.sealValidator = NewSealValidator(s.State, s.HeadersDB, s.IndexDB, s.ResultsDB, s.SealsDB,
		s.Assigner, instance, s.metrics)
	...
	newBlock := s.makeBlockSealingResult(b2, &receipt.ExecutionResult, 1)
	metrics := &module.ConsensusMetrics{}
...
func (s *SealValidationSuite) TestSealEmergencySeal_VerificationRequire1ApprovalReceiveEmergencyNotTriggered() {
	instance, err := updatable_configs.NewSealingConfigs(1, 1, 3, false)
	require.NoError(s.T(), err)
	s.sealValidator = NewSealValidator(s.State, s.HeadersDB, s.IndexDB, s.ResultsDB, s.SealsDB,
		s.Assigner, instance, s.metrics)
	...
	newBlock := s.makeBlockSealingResult(b2, &receipt.ExecutionResult, 0)
	metrics := &module.ConsensusMetrics{}
	metrics.On("EmergencySeal").Run(func(args mock.Arguments) {
...
func (s *SealValidationSuite) TestSealInvalidChunkSignersCount() {
	_, _, newBlock, _, seal := s.generateBasicTestFork()
	seal.AggregatedApprovalSigs[0].SignerIDs = seal.AggregatedApprovalSigs[0].SignerIDs[1:] // modify seal (seal pointer already included in newBlock's payload)
...
func (s *SealValidationSuite) TestSealInvalidChunkSignaturesCount() {
	...
	// modify seal (seal pointer already included in newBlock's payload)
...
func (s *SealValidationSuite) TestSealInvalidChunkAssignment() {
	_, _, newBlock, _, seal := s.generateBasicTestFork()
	seal.AggregatedApprovalSigs[0].SignerIDs[0] = unittest.IdentifierFixture() // modify seal (seal pointer already included in newBlock's payload)
...
func (s *SealValidationSuite) TestHighestSeal() {
	...
	block3 := unittest.BlockWithParentFixture(s.LatestFinalizedBlock.ToHeader())
	...
	block6 := unittest.BlockWithParentFixture(block5.ToHeader())
...
func (s *SealValidationSuite) TestValidatePayload_SealsSkipBlock() {
	...
	s.T().Run("no seal for the immediately next unsealed block", func(t *testing.T) {
		X := unittest.BlockWithParentFixture(b4.ToHeader())
...
func (s *SealValidationSuite) TestValidatePayload_ExecutionDisconnected() {
	...
	s.T().Run("seals in candidate block does connect to latest sealed result of parent", func(t *testing.T) {
		X := unittest.BlockWithParentFixture(blocks[4].ToHeader())
		X.SetPayload(flow.Payload{
...
func (s *SealValidationSuite) TestExtendSealDuplicate() {
	...
	s.T().Run("Duplicate seal in separate block", func(t *testing.T) {
		...
		// by itself, b3 should be a valid extension
		_, err := s.sealValidator.Validate(b3)
		require.NoError(t, err)
		s.Extend(b3)

		// insert B4 with a duplicate seal
		b4 := unittest.BlockWithParentFixture(b3.ToHeader())
...
func (s *SealValidationSuite) TestExtendSeal_NoIncorporatedResult() {
	...
	b1 := unittest.BlockWithParentFixture(s.LatestFinalizedBlock.ToHeader())
	b2 := unittest.BlockWithParentFixture(b1.ToHeader())
	s.Extend(b1)
	s.Extend(b2)
	...
	newBlock := unittest.BlockWithParentFixture(b2.ToHeader())
	newBlock.SetPayload(flow.Payload{
...
func (s *SealValidationSuite) TestExtendSeal_ResultIncorporatedOnDifferentFork() {
	...
	// construct block A1
	a1 := unittest.BlockWithParentFixture(s.LatestFinalizedBlock.ToHeader())
...
func (s *SealValidationSuite) makeBlockSealingResult(parentBlock *flow.Block, sealedResult *flow.ExecutionResult, numberApprovals int) *flow.Block {
	...
	sealingBlock := unittest.BlockWithParentFixture(parentBlock.ToHeader())
