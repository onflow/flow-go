...
func TestState_InteractionMeasuring(t *testing.T) {
	view := utils.NewSimpleView()
	mh := weighted.NewMeteringHandler(0, 0, math.MaxUint64)
	m := weighted.NewMeter(mh)
	st := state.NewState(view, state.WithMeter(m))
	...
	require.NoError(t, err)
	require.Equal(t, uint64(0), m.ReadCounter())
	require.Equal(t, uint64(0), m.TotalBytesRead())
	require.Equal(t, uint64(1), m.WriteCounter())
	require.Equal(t, size, m.TotalBytesWritten())
	...
	require.NoError(t, err)
	require.Equal(t, keySize, m.TotalBytesRead())
}
...
func TestState_MaxInteraction(t *testing.T) {
	...
	view := utils.NewSimpleView()
	mh := weighted.NewMeteringHandler(0, 0, 12)
	m := weighted.NewMeter(mh)
	st := state.NewState(view, state.WithMeter(m))

	// read - interaction 3
	_, err := st.Get("1", "2", "3")
	require.Equal(t, m.TotalInteractionUsed(), uint64(3))
	require.NoError(t, err)

	// read - interaction 12
	_, err = st.Get("123", "234", "345")
	require.Equal(t, m.TotalInteractionUsed(), uint64(12))
	require.NoError(t, err)

	// read - interaction 21
	_, err = st.Get("234", "345", "456")
	require.Equal(t, m.TotalInteractionUsed(), uint64(21))
	require.Error(t, err)

	mh = weighted.NewMeteringHandler(0, 0, 9)
	m = weighted.NewMeter(mh)
	st = state.NewState(view, state.WithMeter(m))
	stChild := st.NewChild()
	...
	require.NoError(t, err)
	require.Equal(t, m.TotalInteractionUsed(), uint64(0))

	// commit
	err = st.MergeState(stChild)
	require.NoError(t, err)
	require.Equal(t, m.TotalInteractionUsed(), uint64(4))
	...
	_, err = st.Get("2", "3", "4")
	require.NoError(t, err)
	require.Equal(t, m.TotalInteractionUsed(), uint64(7))
