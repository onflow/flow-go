...
var signatureVerifier = crypto.DefaultSignatureVerifier{}

type TransactionSignatureVerifier struct{}

func NewTransactionSignatureVerifier() *TransactionSignatureVerifier {
	return &TransactionSignatureVerifier{}
}

func (v TransactionSignatureVerifier) Verify(
	ctx *context.Context,
	...
	if parentSpan != nil {
		span := ctx.Tracer.StartSpanFromParent(parentSpan, trace.FVMVerifyTransaction)
		span.LogFields(
	...
	for _, addr := range tx.Authorizers {
		...
		if !v.hasSufficientKeyWeight(payloadWeights, addr, ctx.AccountKeyWeightThreshold) {
			msg := fmt.Sprintf("authorizer account does not have sufficient signatures (%d < %d)", payloadWeights[addr], ctx.AccountKeyWeightThreshold)
			return errors.NewAccountAuthorizationErrorf(addr, msg)
		}
	}

	if !v.hasSufficientKeyWeight(envelopeWeights, tx.Payer, ctx.AccountKeyWeightThreshold) {
		// TODO change this to payer error (needed for fees)
		msg := fmt.Sprintf("payer account does not have sufficient signatures (%d < %d)", envelopeWeights[tx.Payer], ctx.AccountKeyWeightThreshold)
...
func (v TransactionSignatureVerifier) verifyAccountSignature(
	...
	validWithoutTag, err := signatureVerifier.Verify(
		txSig.Signature,
		nil,
		message,
	...
	validWithTag, err := signatureVerifier.Verify(
		txSig.Signature,
...
func (TransactionSignatureVerifier) hasSufficientKeyWeight(
	weights map[flow.Address]int,
	address flow.Address,
	threshold int,
) bool {
	return weights[address] >= threshold
}

func (TransactionSignatureVerifier) sigIsForProposalKey(
	txSig flow.TransactionSignature,
	proposalKey flow.ProposalKey,
) bool {
