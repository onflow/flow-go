...
func (e *TransactionEnv) SetTraceSpan(span opentracing.Span) {
	e.traceSpan = span
}

func (e *TransactionEnv) isTraceable() bool {
	return e.traceSpan != nil
}
...
func (e *TransactionEnv) GetStorageCapacity(address common.Address) (capacity uint64, err error) {
	if e.isTraceable() {
...
func (e *TransactionEnv) GetAccountBalance(address common.Address) (balance uint64, err error) {
	if e.isTraceable() {
...
func (e *TransactionEnv) GetAccountAvailableBalance(address common.Address) (available uint64, err error) {
	if e.isTraceable() {
		sp := e.ctx.Tracer.StartSpanFromParent(e.traceSpan, trace.FVMEnvGetAccountBalance)
		defer sp.Finish()
	}
...
func (e *TransactionEnv) HighLevelStorageEnabled() bool {
	return e.ctx.SetValueHandler != nil
}

func (e *TransactionEnv) SetCadenceValue(owner common.Address, key string, value cadence.Value) error {
...
func (e *TransactionEnv) GetAccountKey(address runtime.Address, index int) (*runtime.AccountKey, error) {
	if e.isTraceable() {
...
func (e *TransactionEnv) RevokeAccountKey(address runtime.Address, index int) (*runtime.AccountKey, error) {
	if e.isTraceable() {
		sp := e.ctx.Tracer.StartSpanFromParent(e.traceSpan, trace.FVMEnvRemoveAccountKey)
		defer sp.Finish()
	}

	return e.accountKeys.RevokeAccountKey(address, index)
}
