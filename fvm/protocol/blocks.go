...
func NewBlockFinder(storage storage.Headers) *BlocksFinder {
	return &BlocksFinder{storage: storage}
}

// ByHeightFrom returns the block header by height.
func (b *BlocksFinder) ByHeightFrom(height uint64, header *flow.Header) (*runtime.Block, error) {
	if header == nil {
		byHeight, err := b.storage.ByHeight(height)
		if err != nil {
			return nil, err
		}
		return runtimeBlockFromHeader(byHeight), nil
	}

	if header.Height == height {
		return runtimeBlockFromHeader(header), nil
	}
	...
	for {
		...
		if parent.Height == height {
			return runtimeBlockFromHeader(parent), nil
		}
		...
		bl, err := b.storage.ByHeight(height)
		if err != nil {
			return nil, err
		}

		return runtimeBlockFromHeader(bl), nil
	}
}
