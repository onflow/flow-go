...
transaction {
    ...
    prepare(signer: auth(Storage, Capabilities) &Account) {
        ...
        if signer.storage.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault) == nil {
            ...
            let receiverCap = signer.capabilities.storage.issue<&{FungibleToken.Receiver, FungibleToken.Vault}>(/storage/flowTokenVault)
            signer.capabilities.publish(receiverCap, at: /public/flowTokenReceiver)

            // Create a public capability to the Vault that only exposes
            // the balance field through the Balance interface
            let balanceCap = signer.capabilities.storage.issue<&{FungibleToken.Balance, FungibleToken.Vault}>(/storage/flowTokenVault)
