	...
	return stdlib.StandardLibraryValue{
		...
		Value: interpreter.NewUnmeteredStaticHostFunctionValue(
			...
			func(invocation interpreter.Invocation) interpreter.Value {
				...
				expectedArgumentCount := len(randomSourceFunctionType.Parameters)
...
// transactionIndexFunctionType is the type of the `transactionIndex` function.
...
func transactionIndexDeclaration(renv *ReusableCadenceRuntime) stdlib.StandardLibraryValue {
	return stdlib.StandardLibraryValue{
		...
		Value: interpreter.NewUnmeteredStaticHostFunctionValue(
			...
			func(invocation interpreter.Invocation) interpreter.Value {
				actualArgumentCount := len(invocation.Arguments)
				expectedArgumentCount := len(transactionIndexFunctionType.Parameters)
