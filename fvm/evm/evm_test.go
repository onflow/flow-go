	...
	t.Run("testing EVM.runTxAs (happy case)", func(t *testing.T) {
		...
		RunWithNewEnvironment(t,
			chain, func(
				...
				require.Equal(t, uint64(130_000), directCall.GasLimit)
			},
	...
	t.Run("test dry run clear current value", func(t *testing.T) {
		RunWithNewEnvironment(t,
			chain, func(
				...
				gasLimit := dryRunResult.GasConsumed + gethParams.StorageWriteAmsterdam
				innerTxBytes = testAccount.PrepareSignAndEncodeTx(t,
					testContract.DeployedAt.ToCommon(),
					data,
					big.NewInt(0),
	...
	t.Run("test EVM.dryRun is properly metered", func(t *testing.T) {
		RunWithNewEnvironment(t,
			chain, func(
				...
				require.NoError(t, err)
				require.NoError(t, output.Err)
				assert.Equal(t, uint64(158), output.ComputationUsed)
	...
	t.Run("test dryCall validation error", func(t *testing.T) {
		RunWithNewEnvironment(t,
			chain, func(
				...
				assert.Equal(t, uint64(15_331), result.GasConsumed)
			})
	})
	...
	t.Run("happy case", func(t *testing.T) {
		RunContractWithNewEnvironment(
			...
			func(
				...
				//
				require.NotEmpty(t, blockEventPayload.Hash)
				require.Equal(t, uint64(3_574_480), blockEventPayload.TotalGasUsed)
