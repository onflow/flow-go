...
func (_m *BlockContext) ExecuteScript(ledger fvm.Ledger, script []byte) (*fvm.ScriptResult, error) {
	ret := _m.Called(ledger, script)

	var r0 *fvm.ScriptResult
	if rf, ok := ret.Get(0).(func(fvm.Ledger, []byte) *fvm.ScriptResult); ok {
		r0 = rf(ledger, script)
	} else {
		if ret.Get(0) != nil {
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(fvm.Ledger, []byte) error); ok {
		r1 = rf(ledger, script)
	} else {
		r1 = ret.Error(1)
	}
