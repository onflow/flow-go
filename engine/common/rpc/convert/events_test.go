...
func TestConvertEventWithoutPayloadConversion(t *testing.T) {
	...
	t.Run("convert empty event", func(t *testing.T) {
		event := unittest.EventFixture(flow.EventAccountCreated, 2, 3, txID, []byte{})
...
func TestConvertEventWithPayloadConversion(t *testing.T) {
	...
	var err error
	ccfEvent := unittest.EventFixture(flow.EventAccountCreated, 2, 3, txID, []byte{})
	ccfEvent.Payload, err = ccf.Encode(cadenceValue)
	require.NoError(t, err)

	jsonEvent := unittest.EventFixture(flow.EventAccountCreated, 2, 3, txID, []byte{})
...
func TestConvertEvents(t *testing.T) {
	...
	for i := 0; i < eventCount; i++ {
		...
		event := unittest.EventFixture(flow.EventAccountCreated, 2, eventIndex, txID, []byte{})
		ccfEvent := unittest.EventFixture(flow.EventAccountCreated, 2, eventIndex, txID, []byte{})
		jsonEvent := unittest.EventFixture(flow.EventAccountCreated, 2, eventIndex, txID, []byte{})
