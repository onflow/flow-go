...
func (ss *SyncSuite) SetupTest() {
	...
	ss.net.On("RegisterDirectMessageHandler", mock.Anything, mock.Anything).Return(nil)
	...
	e, err := New(log, metrics, ss.net, ss.comp, ss.core, finalizedHeader,
...
func (ss *SyncSuite) TestOnSyncRequest() {
	...
	ss.net.AssertNotCalled(ss.T(), "SendDirectMessage", mock.Anything, mock.Anything, mock.Anything)
	...
	ss.core.On("HandleHeight", ss.head, req.Height)
	ss.core.On("WithinTolerance", ss.head, req.Height).Return(false)
	ss.net.On("SendDirectMessage", mock.Anything, mock.Anything, mock.Anything).Return(nil).Run(
		func(args mock.Arguments) {
			res := args.Get(1).(*messages.SyncResponse)
			...
			recipientID := args.Get(2).(flow.Identifier)
...
func (ss *SyncSuite) TestOnRangeRequest() {
	...
	ss.net.AssertNumberOfCalls(ss.T(), "SendDirectMessage", 0)
	...
	ss.net.On(
		"SendDirectMessage",
		engine.SyncCommittee,
		mock.AnythingOfType("*messages.BlockResponse"),
		mock.AnythingOfType("flow.Identifier"),
	).Return(nil).Once().Run(
		func(args mock.Arguments) {
			res := args.Get(1).(*messages.BlockResponse)
...
func (ss *SyncSuite) TestPollHeight() {
	...
	for _, other := range others {
		ss.net.On(
			"SendDirectMessage",
			engine.SyncCommittee,
			mock.AnythingOfType("*messages.SyncRequest"),
			other.NodeID,
		).Return(nil).Run(
			func(args mock.Arguments) {
				req := args.Get(1).(*messages.SyncRequest)
				require.Equal(ss.T(), ss.head.Height, req.Height, "request should contain finalized height")
			},
		)
	}
	ss.e.pollHeight()
	ss.net.AssertExpectations(ss.T())
}
...
func (ss *SyncSuite) TestSendRequests() {
	...
	ss.net.On(
		"SendDirectMessage",
		engine.SyncCommittee,
		mock.AnythingOfType("*messages.RangeRequest"),
		mock.AnythingOfType("flow.Identifier"),
	).Return(nil).Run(
		func(args mock.Arguments) {
			req := args.Get(1).(*messages.RangeRequest)
	...
	ss.net.On(
		"SendDirectMessage",
		engine.SyncCommittee,
		mock.AnythingOfType("*messages.BatchRequest"),
		mock.AnythingOfType("flow.Identifier"),
	).Return(nil).Run(
		func(args mock.Arguments) {
			req := args.Get(1).(*messages.BatchRequest)
...
func (ss *SyncSuite) TestStartStop() {
	...
	unittest.AssertReturnsBefore(ss.T(), func() {
		ss.e.finalizedHeader.Start(signalerCtx)
		ss.e.Start(signalerCtx)
		<-ss.e.Ready()
		cancel()
		<-ss.e.Done()
	}, time.Second)
...
func (ss *SyncSuite) TestProcessingMultipleItems() {
	...
	for i := 0; i < 5; i++ {
		...
		ss.core.On("HandleHeight", mock.Anything, msg.Height).Once()
		ss.net.On("SendDirectMessage", mock.Anything, mock.Anything, mock.Anything).Return(nil)
