...
func (c *ExecutionResultContainer) AddReceipt(receipt *flow.ExecutionReceipt) (uint, error) {
	if receipt.ExecutionResult.ID() != c.resultID {
		return 0, fmt.Errorf("receipt is for result %v, while ExecutionResultContainer pertains to result %v: %w", receipt.ExecutionResult.ID(), c.resultID, ErrIncompatibleReceipt)
	}
