...
func (_m *API) GetRegisterValues(registerIDs flow.RegisterIDs, height uint64, criteria optimistic_sync.Criteria) ([][]byte, *access.ExecutorMetadata, error) {
	...
	var r2 error
	if rf, ok := ret.Get(0).(func(flow.RegisterIDs, uint64, optimistic_sync.Criteria) ([][]byte, *access.ExecutorMetadata, error)); ok {
		return rf(registerIDs, height, criteria)
	}
	if rf, ok := ret.Get(0).(func(flow.RegisterIDs, uint64, optimistic_sync.Criteria) [][]byte); ok {
