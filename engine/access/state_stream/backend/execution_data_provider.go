...
type ExecutionDataProvider interface {
	ExecutionDataByBlockHeight(
		ctx context.Context,
		height uint64,
	) (*execution_data.BlockExecutionDataEntity, error)

	ExecutionDataByExecutionResultID(
		ctx context.Context,
		height uint64,
		id flow.Identifier,
	) (*execution_data.BlockExecutionDataEntity, error)
...
func (e *ExecutionDataProviderImpl) ExecutionDataByExecutionResultID(
	ctx context.Context,
	height uint64,
	...
	execData, err := e.execDataCache.ByExecutionResultID(ctx, execResultID)
	if err != nil {
		if errors.Is(err, storage.ErrNotFound) ||
			execution_data.IsBlobNotFoundError(err) {
			err = errors.Join(err, subscription.ErrBlockNotReady)
