...
func (s *Suite) initIngestionEngine(ctx irrecoverable.SignalerContext) *Engine {
	...
	eng, err := New(s.log, s.net, s.proto.state, s.me, s.request, s.blocks, s.headers, s.collections, s.transactions, s.results, s.receipts, s.collectionExecutedMetric, processedHeight, s.lastFullBlockHeight, nil)
	require.NoError(s.T(), err)
