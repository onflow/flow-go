...
func TestScripts(t *testing.T) {
	...
	t.Run("get by Latest height", func(t *testing.T) {
		...
		req := scriptReq("", router.SealedHeightQueryParam, validBody)
		router.AssertOKResponse(t, req, fmt.Sprintf(
			"\"%s\"",
	...
	t.Run("get invalid", func(t *testing.T) {
		...
		for _, test := range tests {
			...
			router.AssertResponse(t, req, http.StatusBadRequest, test.out, backend)
		}
	})
}
