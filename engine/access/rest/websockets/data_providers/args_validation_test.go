...
func TestExtractHeartbeatInterval(t *testing.T) {
	def := uint64(42)
	// missing -> default
	v, err := extractHeartbeatInterval(wsmodels.Arguments{}, def)
	require.NoError(t, err)
	require.Equal(t, def, v)
	// bad type -> ErrInvalidArgument
	_, err = extractHeartbeatInterval(wsmodels.Arguments{"heartbeat_interval": 7}, def)
	require.Error(t, err)
	require.ErrorIs(t, err, ErrInvalidArgument)
	// bad parse -> ErrInvalidArgument
	_, err = extractHeartbeatInterval(wsmodels.Arguments{"heartbeat_interval": "NaN"}, def)
	require.Error(t, err)
	require.ErrorIs(t, err, ErrInvalidArgument)
	// ok
	v, err = extractHeartbeatInterval(wsmodels.Arguments{"heartbeat_interval": "100"}, def)
	require.NoError(t, err)
