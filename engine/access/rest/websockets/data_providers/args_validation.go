...
//
// Expected errors:
//   - [data_providers.ErrInvalidArgument]: A client supplied an invalid or unsupported argument.
//
// All validation errors produced by functions like ensureAllowedFields, extractArrayOfStrings,
// extractHeartbeatInterval, and extractExecutionStateQueryFields must satisfy
// errors.Is(err, ErrInvalidArgument). This allows callers and tests to reliably classify
// client-input errors without relying on exact error message matching.
var ErrInvalidArgument = errors.New("invalid argument")

// ensureAllowedFields verifies that the provided fields map does not contain keys
// outside of the provided allowedFields set.
//
// Expected errors:
...
//   - agreeingExecutorsCount: parsed uint64 (0 if missing).
//   - requiredExecutorIDs: parsed from hex strings (nil if missing).
//   - includeExecutorMetadata: parsed boolean (false if missing).
//
// All validation errors returned by this function will satisfy errors.Is(err, data_providers.ErrInvalidArgument).
//
// Expected errors:
