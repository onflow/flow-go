...
func (s *TransactionStatusesProviderSuite) subscribeTransactionStatusesDataProviderTestCases(backendResponses []*accessmodel.TransactionResult) []testType[[]*accessmodel.TransactionResult, any] {
	expectedResponses := s.expectedTransactionStatusesResponses(backendResponses, TransactionStatusesTopic)

	return []testType[[]*accessmodel.TransactionResult, any]{
		{
...
func (s *TransactionStatusesProviderSuite) TestMessageIndexTransactionStatusesProviderResponse_HappyPath() {
	...
	txStatusesChan := make(chan []*accessmodel.TransactionResult)

	// Create a mock subscription and mock the channel
	sub := submock.NewSubscription[[]*accessmodel.TransactionResult](s.T())
	sub.On("Channel").Return((<-chan []*accessmodel.TransactionResult)(txStatusesChan))
