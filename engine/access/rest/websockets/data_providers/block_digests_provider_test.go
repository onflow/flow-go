	...
	testHappyPath(
		...
		func(dataChan chan interface{}) {
			for _, block := range s.blocks {
				dataChan <- flow.NewBlockDigest(block.ID(), block.Header.Height, time.UnixMilli(int64(block.Header.Timestamp)).UTC())
			}
		},
...
func (s *BlockDigestsProviderSuite) validBlockDigestsArgumentsTestCases() []testType {
	...
	for i, b := range s.blocks {
		blockDigest := flow.NewBlockDigest(b.ID(), b.Header.Height, time.UnixMilli(int64(b.Header.Timestamp)).UTC())
