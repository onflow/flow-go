...
func (suite *Suite) TestExecutionNodesForBlockID() {
	...
	block := unittest.BlockFixture()
	allExecutionNodes := make(flow.IdentityList, 0, totalReceipts) // assume each ER is generated by a unique exec node

	// generate identitylist and receipts for each block and mark each receipt with a unique execution ID
	ids := unittest.IdentityListFixture(totalReceipts, unittest.WithRole(flow.RoleExecution))
	allExecutionNodes = append(allExecutionNodes, ids...)

	// same execution result for all receipts for this block
	executionResult := unittest.ExecutionResultFixture()
	receipts := make([]*flow.ExecutionReceipt, totalReceipts)
