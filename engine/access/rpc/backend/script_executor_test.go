...
//
//	log            zerolog.Logger
//	registerIndex  storage.RegisterSnapshotReader
...
//	s.Require().NoError(err)
//	s.registerIndex = pebbleStorage.NewRegisterSnapshotReader(pebbleRegisters)
//
...
//
//		err = scriptExec.Initialize(s.scripts, s.versionControl)
//		s.Require().NoError(err)
//
//		// Execute the script at the specified block height
//		res, err := scriptExec.ExecuteAtBlockHeight(ctx, script, scriptArgs, s.height)
