...
func TestContructor(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			filter, err := state_stream.NewEventFilter(state_stream.DefaultEventFilterConfig,
				chain,
				test.eventTypes,
				test.addresses, test.contracts)
			if test.err {
				assert.Error(t, err)
...
func TestFilter(t *testing.T) {
	...
	filter, err := state_stream.NewEventFilter(state_stream.DefaultEventFilterConfig, chain,
		[]string{"flow.AccountCreated", "A.0000000000000001.Contract1.EventA"}, nil, nil)
	assert.NoError(t, err)
