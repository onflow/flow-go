...
import (
	...
	"github.com/onflow/flow-go/engine/access/api/rest/common/middleware"
...
func requestURL(t *testing.T, ids []string, start string, end string, expandResponse bool, selectedFields []string, heights ...string) *http.Request {
	...
	if expandResponse {
		var expands []string
		expands = append(expands, router.ExpandableFieldPayload)
		expands = append(expands, router.ExpandableExecutionResult)
