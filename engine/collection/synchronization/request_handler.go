...
func (r *RequestHandlerEngine) onRangeRequest(originID flow.Identifier, req *messages.RangeRequest) error {
	...
	// send the response
	res := &messages.ClusterRangeResponse{
		Nonce:  req.Nonce,
		Blocks: blocks,
	}
	err = r.con.Unicast(res, originID)
	...
	r.metrics.MessageSent(metrics.EngineClusterSynchronization, metrics.MessageRangeResponse)
...
func (r *RequestHandlerEngine) onBatchRequest(originID flow.Identifier, req *messages.BatchRequest) error {
	...
	r.metrics.MessageSent(metrics.EngineClusterSynchronization, metrics.MessageBatchResponse)
