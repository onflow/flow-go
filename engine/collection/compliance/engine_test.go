...
func (cs *ComplianceSuite) TestBroadcastProposalWithDelay() {
	...
	cs.hotstuff.On("SubmitProposal", block.Header, parent.Header.View).Return(make(<-chan struct{})).Once()
...
func (cs *ComplianceSuite) TestSubmittingMultipleEntries() {
	...
	go func() {
		...
		cs.hotstuff.On("SubmitProposal", block.Header, cs.head.Header.View).Return(make(<-chan struct{}))
