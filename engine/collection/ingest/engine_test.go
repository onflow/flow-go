...
func TestInvalidTransaction(t *testing.T) {
	...
	t.Run("expired reference block ID", func(t *testing.T) {
		util.RunWithStorageLayer(t, func(_ *badger.DB, _ *storage.Headers, _ *storage.Guarantees, _ *storage.Seals, _ *storage.Index, _ *storage.Payloads, blocks *storage.Blocks) {
