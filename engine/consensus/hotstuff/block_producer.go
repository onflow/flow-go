...
// NewBlockProducer creates an instance of BlockProducer
...
func (bp *BlockProducer) signBlockProposal(proposal *types.Block) (*types.BlockProposal, error) {
	...
	// convert the proposal to a vote, because signature of a proposal will be used as a vote.
	vote := proposal.ToVote()

	// sign the vote
	sig := bp.signer.SignVote(vote, selfIdx)
