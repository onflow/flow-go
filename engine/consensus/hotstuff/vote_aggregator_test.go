...
func TestOnlyReceiveBlock(t *testing.T) {
	...
	qc, built, err := va.BuildQCOnReceivedBlock(block)
	require.Nil(t, qc)
...
func newMockBlock(view uint64) *hotstuff.Block {
...
func newMockVote(view uint64, blockID flow.Identifier, signerIndex uint32) *hotstuff.Vote {
	vote := &hotstuff.Vote{
		BlockID: blockID,
		View:    view,
	}
...
func newMockVoteAggregator(t *testing.T) *VoteAggregator {
	...
	return NewVoteAggregator(&mockdist.Consumer{}, 0, viewState, voteValidator, SigAggregator())
}
