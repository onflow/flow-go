...
func (f *BlockBuilder) Blocks() ([]*hotstuff.Block, error) {
	blocks := make([]*hotstuff.Block, 0, len(f.blockViews))
	...
	qcs := make(map[string]*hotstuff.QuorumCertificate)
	...
	for _, bv := range f.blockViews {
		...
		qcs[bv.BlockIndex()] = &hotstuff.QuorumCertificate{
			View:                block.View,
...
func makePayloadHash(view uint64, qc *hotstuff.QuorumCertificate, blockVersion int) flow.Identifier {
...
func makeBlockID(block *hotstuff.Block) flow.Identifier {
	return flow.MakeID(struct {
		View        uint64
		QC          *hotstuff.QuorumCertificate
...
func makeGenesis() *forks.BlockQC {
	genesis := &hotstuff.Block{
		View: 1,
	}
	genesis.BlockID = makeBlockID(genesis)

	genesisQC := &hotstuff.QuorumCertificate{
		View:    1,
