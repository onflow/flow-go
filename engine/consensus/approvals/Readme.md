...
The decision whether the ER can be emergency sealed is governed by two protocol parameters:
...
1. Let Δh<sub>1</sub> be the height difference between the latest finalized block and block `A`.
   ...
   * This means that block `A` must have at least `DefaultEmergencySealingThresholdForFinalization` unsealed but
...
3. There must be consistent execution results from at least two different Execution Nodes to even consider
   ...
   * Comment: as a temporary safety measure, requiring at least two consistent results must hold for any block,
