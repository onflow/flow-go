...
func TestDKG(t *testing.T) {
	t.Run("5nodes", func(t *testing.T) { testDKG(t, 5, time.Second) })
	t.Run("10nodes", func(t *testing.T) { testDKG(t, 10, time.Second) })
	// t.Run("20nodes", func(t *testing.T) { testDKG(t, 20, 5*time.Second) })
}

func testDKG(t *testing.T, n int, phaseDuration time.Duration) {
	nodes := initNodes(t, n, phaseDuration)
...
func TestDKGThreshold(t *testing.T) {
	...
	// wait until the nodes are shutdown
	wait(t, nodes[:gn], 5*phaseDuration)

	checkArtifacts(t, nodes[:gn])
}
