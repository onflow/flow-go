...
func GenerateExecutionResultAndChunkDataPacks(
	...
	for i := range result.StateCommitments {
		...
		chunk := GenerateChunk(i, startState, endState, collectionID, blockID, eventsHash, uint64(txNumber))
...
func GenerateChunk(colIndex int,
	...
	colID, blockID, eventsCollection flow.Identifier, txNumber uint64) *flow.Chunk {
	return &flow.Chunk{
