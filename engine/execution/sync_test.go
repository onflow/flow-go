...
// Test 2 ENs can sync execution state with each other.
// It creates 2 ENs, and 5 blocks:
// genesis <- block1 <- block2 <- block3 <- block4 <- block5
// test if EN1 has genesis, and EN2 has block1 and block2,
// then when EN1 receives the orphan blocks (block3 and block4),
// it should trigger state sync, and receives execution state of
// block1 and block2, and then EN1 will execute block1 to block4.
// after block4 has been executed, we exit the state sync, and
// when received block5, we will execute it.
