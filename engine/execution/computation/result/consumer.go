...
type ExecutedCollection interface {
	...
	// Temp method before changing the committer to be dependent on an interface
	ExecutionSnapshot() *state.ExecutionSnapshot

	// ServiceEventList returns a list of service events emitted during this collection
	ServiceEventList() flow.ServiceEventList
}
...
type AttestedCollection interface {
	...
	// StateDeltaCommitment()  flow.StateCommitment
