...
func (_m *BlockComputer) ExecuteBlock(ctx context.Context, parentBlockExecutionResultID flow.Identifier, block *entity.ExecutableBlock, snapshot state.StorageSnapshot, derivedBlockData *derived.DerivedBlockData) (*execution.ComputationResult, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, flow.Identifier, *entity.ExecutableBlock, state.StorageSnapshot, *derived.DerivedBlockData) (*execution.ComputationResult, error)); ok {
		return rf(ctx, parentBlockExecutionResultID, block, snapshot, derivedBlockData)
	}
