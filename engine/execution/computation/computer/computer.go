...
func NewBlockComputer(
	...
	systemChunkASTCache, err := fvm.NewLRUASTCache(64)
	if err != nil {
		return nil
	}
	...
	systemChunkCtx := fvm.NewContextFromParent(
		...
		fvm.WithTransactionProcessors([]fvm.TransactionProcessor{fvm.NewTransactionInvocator()}),
	)
...
func (e *blockComputer) executeBlock(
	...
	collLength := len(collections)
	if e.systemChunk {
		collLength++
	}

	interactions := make([]*delta.Snapshot, collLength)
