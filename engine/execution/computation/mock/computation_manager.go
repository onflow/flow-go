...
import (
	...
	state "github.com/onflow/flow-go/fvm/storage/state"
)
...
func (_m *ComputationManager) ComputeBlock(ctx context.Context, parentBlockExecutionResultID flow.Identifier, block *entity.ExecutableBlock, snapshot state.StorageSnapshot) (*execution.ComputationResult, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, flow.Identifier, *entity.ExecutableBlock, state.StorageSnapshot) (*execution.ComputationResult, error)); ok {
		return rf(ctx, parentBlockExecutionResultID, block, snapshot)
	}
...
func (_m *ComputationManager) ExecuteScript(ctx context.Context, script []byte, arguments [][]byte, blockHeader *flow.Header, snapshot state.StorageSnapshot) ([]byte, error) {
	...
	var r0 []byte
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, []byte, [][]byte, *flow.Header, state.StorageSnapshot) ([]byte, error)); ok {
		return rf(ctx, script, arguments, blockHeader, snapshot)
	}
