...
var DefaultConfig = PruningConfig{
	...
	BatchSize: 1200,
	// when choosing a value, consider the batch size and block time, for instance,
	// the block time is 1.2 block/second, and the batch size is 1200, so the batch commit time is 1200 / 1.2 = 1000 seconds.
	// so the sleep time should be smaller than 1000 seconds, otherwise, the pruner is not able to keep up with the block generation.
