...
func (suite *Suite) TestGetEventsForBlockIDs() {
	...
	for i := range blockIDs {
		...
		for j := range eventsForBlock {
			e := unittest.EventFixture(flow.EventAccountCreated, uint32(j), uint32(j), unittest.IdentifierFixture(), []byte{})
			eventsForBlock[j] = e
...
func (suite *Suite) TestGetTransactionResultsByBlockID() {
	...
	for j := 0; j < len(eventsForTx1); j++ {
		e := unittest.EventFixture(flow.EventAccountCreated, uint32(0), uint32(j), tx1ID, []byte{})
		eventsForTx1[j] = e
		eventsForBlock[j] = e
		convertedEventsForTx1[j] = convert.EventToMessage(e)
	}
	for j := 0; j < len(eventsForTx2); j++ {
		e := unittest.EventFixture(flow.EventAccountCreated, uint32(1), uint32(j), tx2ID, []byte{})
		eventsForTx2[j] = e
