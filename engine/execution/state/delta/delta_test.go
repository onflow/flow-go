...
func TestDelta_RegisterUpdatesAreSorted(t *testing.T) {
	...
	data[1].Key = flow.NewRegisterID("a", "1")
	data[2].Key = flow.NewRegisterID("b", "1")
	data[3].Key = flow.NewRegisterID("b", "1")
	data[4].Key = flow.NewRegisterID("b", "2")
	...
	// set in random order
	d.Set(data[2].Key.Owner, data[2].Key.Key, data[2].Value)
	d.Set(data[1].Key.Owner, data[1].Key.Key, data[1].Value)
	d.Set(data[3].Key.Owner, data[3].Key.Key, data[3].Value)
