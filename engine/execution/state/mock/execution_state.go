...
// PersistExecutionState provides a mock function with given fields: log, ctx, header, endState, chunkDataPacks, executionReceipt, events, serviceEvents, results
func (_m *ExecutionState) PersistExecutionState(log zerolog.Logger, ctx context.Context, header *flow.Header, endState flow.StateCommitment, chunkDataPacks []*flow.ChunkDataPack, executionReceipt *flow.ExecutionReceipt, events []flow.EventsList, serviceEvents flow.EventsList, results []flow.TransactionResult) error {
	ret := _m.Called(log, ctx, header, endState, chunkDataPacks, executionReceipt, events, serviceEvents, results)

	var r0 error
	if rf, ok := ret.Get(0).(func(zerolog.Logger, context.Context, *flow.Header, flow.StateCommitment, []*flow.ChunkDataPack, *flow.ExecutionReceipt, []flow.EventsList, flow.EventsList, []flow.TransactionResult) error); ok {
		r0 = rf(log, ctx, header, endState, chunkDataPacks, executionReceipt, events, serviceEvents, results)
	} else {
		r0 = ret.Error(0)
	}
