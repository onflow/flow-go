...
func (_m *SnapshotReader) BatchRead(ids []flow.RegisterID) (flow.RegisterIterator, error) {
	ret := _m.Called(ids)

	var r0 flow.RegisterIterator
	if rf, ok := ret.Get(0).(func([]flow.RegisterID) flow.RegisterIterator); ok {
		r0 = rf(ids)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(flow.RegisterIterator)
		}
	}
