...
Notes:
* I am aware that this proposal has some areas of overlap with [FLIP 343](https://github.com/onflow/flow/pull/343), and in general I don't think that they are incompatible. Engines still can (and should) internally queue (or drop) messages from the network. If they do queue them, the `Context` should be queued alongside it. When the message is dequeued, the engine can check the `Context` to see whether the message is still valid. 
