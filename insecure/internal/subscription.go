...
func NewCorruptSubscription(s *pubsub.Subscription) p2p.Subscription {
...
func (c *CorruptSubscription) Next(ctx context.Context) (*pubsub.Message, error) {
	...
	// we read a pubsub.Message from the pubsub.Subscription, however, we need to return
	// a pubsub.Message to the caller of this function, so we need to convert the pubsub.Message.
	// Flow codebase uses the original libp2p pubsub module, and the pubsub.Message is defined
	// in the original libp2p pubsub module, so we cannot use the pubsub.Message in the Flow codebase.
