...
func (c *Conduit) Publish(event interface{}, targetIDs ...flow.Identifier) error {
	...
	err := c.egressController.HandleIncomingEvent(event, c.channel, insecure.Protocol_PUBLISH, 0, targetIDs...)
	if err != nil {
...
func (c *Conduit) Unicast(event interface{}, targetID flow.Identifier) error {
	...
	err := c.egressController.HandleIncomingEvent(event, c.channel, insecure.Protocol_UNICAST, 0, targetID)
	if err != nil {
...
func (c *Conduit) Multicast(event interface{}, num uint, targetIDs ...flow.Identifier) error {
	...
	err := c.egressController.HandleIncomingEvent(event, c.channel, insecure.Protocol_MULTICAST, uint32(num), targetIDs...)
	if err != nil {
