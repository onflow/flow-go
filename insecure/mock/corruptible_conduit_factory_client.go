...
func (_m *CorruptibleConduitFactoryClient) RegisterAttacker(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (insecure.CorruptibleConduitFactory_RegisterAttackerClient, error) {
	_va := make([]interface{}, len(opts))
	for _i := range opts {
		_va[_i] = opts[_i]
	}
	...
	if rf, ok := ret.Get(0).(func(context.Context, *emptypb.Empty, ...grpc.CallOption) insecure.CorruptibleConduitFactory_RegisterAttackerClient); ok {
		r0 = rf(ctx, in, opts...)
	} else {
		if ret.Get(0) != nil {
...
// NewCorruptibleConduitFactoryClient creates a new instance of CorruptibleConduitFactoryClient. It also registers the testing.TB interface on the mock and a cleanup function to assert the mocks expectations.
func NewCorruptibleConduitFactoryClient(t testing.TB) *CorruptibleConduitFactoryClient {
