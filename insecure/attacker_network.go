...
// AttackerNetwork represents the networking interface that is available to the attack orchestrator for sending messages "through"
...
type AttackerNetwork interface {
	component.Component
	...
	// "Inbound" message means it's coming into the attacker network (either from a corrupt node, for an egress message OR
	...
	// dispatches the message to the attacker network through a remote call to this method.
	Observe(*Message)
}
