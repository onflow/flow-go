...
//  1. Orchestrator corrupts the result of the first incoming execution receipt from any of the corrupt execution node.
//  2. Orchestrator sends the corrupt execution result to all corrupt execution nodes.
//  3. If Orchestrator receives any chunk data pack request for a corrupt chunk from a corrupt verification node, it prompts the verifier to approve that chunk by sending it an attestation
//     for that corrupt chunk.
//  4. If Orchestrator receives any chunk data pack response from a corrupt execution node to an honest verification node, it drops the response
//     if it is for one of the corrupt chunks.
//  5. If Orchestrator receives a result approval for the original result (i.e., conflicting result with corrupt result),
//     from a corrupt VN, it drops it, hence no conflicting result with the corrupt result is getting approved by any
//     corrupt VN.
...
type Orchestrator struct {
	...
	corruptNodeIds flow.IdentifierList // identifier of corrupt nodes.
	allNodeIds     flow.IdentityList   // identity of all nodes in the network (including non-corrupt ones)
...
// Register sets the attacker network of the orchestrator.
func (o *Orchestrator) Register(network insecure.AttackerNetwork) {
	o.network = network
}
...
func (o *Orchestrator) HandleEgressEvent(event *insecure.EgressEvent) error {
	...
	case *flow.ExecutionReceipt:
		// orchestrator received execution receipt from corrupt EN after EN executed a block.
		if err := o.handleExecutionReceiptEvent(event); err != nil {
			return fmt.Errorf("could not handle execution receipt event: %w", err)
		}
	case *messages.ChunkDataRequest:
		// orchestrator received chunk data request from corrupt VN after VN assigned a chunk, and VN about to request that chunk to verify it.
	...
	case *messages.ChunkDataResponse:
		// orchestrator received chunk data response from corrupt EN when EN wants to respond to a chunk data request from VN.
	...
	case *flow.ResultApproval:
		// orchestrator receives a result approval from corrupt VN. If it is an approval for the original result, it should
		// be wintermuted, i.e., a corrupt VN must not approve any conflicting result with the corrupt result (otherwise, it
...
// corruptExecutionResult creates a corrupt version of the input receipt by tampering its content so that
// the resulted corrupt version would not pass verification.
...
// handleExecutionReceiptEvent processes incoming execution receipt event from a corrupt execution node.
// If no attack has already been conducted, it corrupts the result of receipt and sends it to all corrupt execution nodes.
...
func (o *Orchestrator) handleExecutionReceiptEvent(receiptEvent *insecure.EgressEvent) error {
	...
	ok := o.corruptNodeIds.Contains(receiptEvent.CorruptOriginId)
	if !ok {
		return fmt.Errorf("sender of the event is not a corrupt node")
	}

	corruptIdentity, ok := o.allNodeIds.ByNodeID(receiptEvent.CorruptOriginId)
	if !ok {
		return fmt.Errorf("could not find corrupt identity for: %x", receiptEvent.CorruptOriginId)
	}

	if corruptIdentity.Role != flow.RoleExecution {
		return fmt.Errorf("wrong sender role for execution receipt: %s", corruptIdentity.Role.String())
	}
	...
	lg := o.logger.With().
		...
		Hex("corrupt_id", logging.ID(receiptEvent.CorruptOriginId)).
	...
	if _, _, conducted := o.AttackState(); conducted {
		...
		if receipt.ExecutionResult.ID() == o.state.originalResult.ID() {
			...
			// corrupt result must have already been sent to this node, so
			// just discard it.
	...
	// replace honest receipt with corrupt receipt
	corruptResult := o.corruptExecutionResult(receipt)

	corruptExecutionIds := o.allNodeIds.Filter(
		filter.And(filter.HasRole(flow.RoleExecution),
			filter.HasNodeID(o.corruptNodeIds...))).NodeIDs()

	// sends corrupt execution result to all corrupt execution nodes.
	for _, corruptExecutionId := range corruptExecutionIds {
		// sets executor id of the result as the same corrupt execution node id that
		// is meant to send this message to the flow network.
		err := o.network.SendEgress(&insecure.EgressEvent{
			CorruptOriginId: corruptExecutionId,
			...
			FlowProtocolEvent: &flow.ExecutionReceipt{ExecutionResult: *corruptResult},
		})
		if err != nil {
			return fmt.Errorf("could not send rpc on channel: %w", err)
		}
		lg.Debug().
			Hex("corrupt_result_id", logging.ID(corruptResult.ID())).
			Hex("corrupt_execution_id", logging.ID(corruptExecutionId)).
			Msg("corrupt result successfully sent to corrupt execution node")
	}

	// saves state of attack for further replies
	o.updateAttackState(&receipt.ExecutionResult, corruptResult)
	lg.Info().
		Hex("corrupt_result_id", logging.ID(corruptResult.ID())).
...
// If request is for a corrupt chunk and comes from a corrupt verification node it is replied with an attestation for that
// chunk.
...
func (o *Orchestrator) handleChunkDataPackRequestEvent(chunkDataPackRequestEvent *insecure.EgressEvent) error {
	ok := o.corruptNodeIds.Contains(chunkDataPackRequestEvent.CorruptOriginId)
	if !ok {
		return fmt.Errorf("sender of the event is not a corrupt node")
	}

	corruptIdentity, ok := o.allNodeIds.ByNodeID(chunkDataPackRequestEvent.CorruptOriginId)
	if !ok {
		return fmt.Errorf("could not find corrupt identity for: %x", chunkDataPackRequestEvent.CorruptOriginId)
	}
	if corruptIdentity.Role != flow.RoleVerification {
		return fmt.Errorf("wrong sender role for chunk data pack request: %s", corruptIdentity.Role.String())
	}
	...
	// Therefore, chunk data request is for an honest result, hence the corrupt verifier can follow
	...
	o.logger.Info().
		Hex("corrupt_id", logging.ID(chunkDataPackRequestEvent.CorruptOriginId)).
...
// handleChunkDataPackResponseEvent wintermutes the chunk data pack reply if it belongs to a corrupt result, and is meant to
...
func (o *Orchestrator) handleChunkDataPackResponseEvent(chunkDataPackReplyEvent *insecure.EgressEvent) error {
	...
	if _, _, conducted := o.AttackState(); conducted {
		...
		ok := o.corruptNodeIds.Contains(chunkDataPackReplyEvent.TargetIds[0])
		if o.state.containsCorruptChunkId(cdpRep.ChunkDataPack.ChunkID) {
			if !ok {
				// this is a chunk data pack response for a CORRUPT chunk to an HONEST verification node
				// we WINTERMUTE it!
				...
				lg.Info().Msg("wintermuted corrupt chunk data response to an honest verification node")
				return nil
			} else {
				// Illegal state! chunk data pack response for a CORRUPT chunk to a CORRUPT verification node.
				// Request must have never been reached to corrupt execution node, and must have been replaced with
				// an attestation.
				lg.Fatal().
					Msg("orchestrator received a chunk data response for corrupt chunk to a corrupt verification node")
			}
		}
	}

	// chunk data response is for an honest result (or before an attack has started), hence the corrupt execution node can follow the protocol and send (pass through) its honest response
	err := o.network.SendEgress(chunkDataPackReplyEvent)
	if err != nil {
		return fmt.Errorf("could not passed through chunk data reply: %w", err)
	}
	o.logger.Debug().
		Hex("corrupt_id", logging.ID(chunkDataPackReplyEvent.CorruptOriginId)).
...
// corrupt verification nodes. Otherwise, it is passed through.
func (o *Orchestrator) handleResultApprovalEvent(resultApprovalEvent *insecure.EgressEvent) error {
	...
	// belongs to the chunks of the original (non-corrupt) result.
...
// the corrupt result of orchestrator's state.
func (o *Orchestrator) replyWithAttestation(chunkDataPackRequestEvent *insecure.EgressEvent) (bool, error) {
	...
	if o.state.containsCorruptChunkId(cdpReq.ChunkID) {
		// requested chunk belongs to corrupt result.
		corruptChunkIndex, err := o.state.corruptChunkIndexOf(cdpReq.ChunkID)
		if err != nil {
			return false, fmt.Errorf("could not find chunk index for corrupt chunk: %w", err)
		}

		attestation := &flow.Attestation{
			BlockID:           o.state.corruptResult.BlockID,
			ExecutionResultID: o.state.corruptResult.ID(),
			ChunkIndex:        corruptChunkIndex,
		}
		...
		if err != nil {
			return false, fmt.Errorf("could not send attestation for corrupt chunk: %w", err)
		}
...
// AttackState returns the corrupt and original execution results involved in this attack.
...
func (o *Orchestrator) AttackState() (flow.ExecutionResult, flow.ExecutionResult, bool) {
	...
	return *o.state.corruptResult, *o.state.originalResult, true
}

func (o *Orchestrator) updateAttackState(originalResult *flow.ExecutionResult, corruptResult *flow.ExecutionResult) {
